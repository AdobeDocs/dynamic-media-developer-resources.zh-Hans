...
`ic` 是一个命令行工具，可将图像文件转换为优化的金字塔TIFF格式(PTIFF)。虽然“图像服务”可以处理图像而无需转换，但建议您将所有大于512x512像素的图像转换为PTIFF。 此转换可确保最佳服务器性能和资源使用，并最大限度地缩短响应时间。

建议对包含照片内容的PTIFF文件进行JPEG编码（指定`-jpegcompress`）。 计算机生成的内容可以从无损压缩（`-deflatecompress`或`-lzwcompress`）中受益。 除非需要颜色转换或像素类型转换，否则JPEG源图像数据将在不解码的情况下传输到PTIFF，以避免质量下降。 在这种情况下，指定的压缩选项仅适用于低分辨率金字塔级。

如果不转换大图像，则不必设置控制要使用多少内存的参数。 但是，如果您是，请使用下面描述的`-maxmem`设置，为`ic`提供更多内存。 计算所需内存量的一条好经验法则是将图像的宽度乘以图像的高度乘以渠道数。 例如，对于Alpha乘以3的RGB图像，为4。 此外，如果渠道是16位/分量，而不是8多次最终结果。
...
<table id="table_E368E220299D449D8311478AB5042987"> 
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>命令选项（请参阅下面）。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>将输出PTIFF文件写入的文件夹。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_02011C7C076745A8BF4378B22C48C8A3"> 
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -deflate压缩  </span> </p> </td> 
  ...
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -autocrop  &lt;&gt; corner  </span>&gt;  &lt;&gt; mode  </span>&gt;  &lt;&gt; tolerance  </span> &lt;&gt;  </span>&gt; infoFile &gt;  </span><span class="varname"><span class="varname"><span class="varname"><span class="varname"> </span></span></span></span></p> </td> 
   <td colname="col2"> <p>计算裁剪矩形以最小化纯色背景。 如果自动裁剪算法会导致整个图像被裁剪，则不会输出裁剪信息。 </p> <p>要在不转换图像的情况下计算裁剪矩形，请指定<span class="codeph"> -autocrop </span>（不带<span class="codeph"> -convert </span>）和<span class="codeph"> <span class="varname"> destFile。</span> </span></p>

<p><i><b>corner</b></i> - ul | | | lr </p>
   <p> 指定要使用种子点的图像的哪个角。 如果模式为1，则忽略。</p>
   <p><i><b>mode</b></i> - 0 | 1</p>
   <p>设置为0可基于指定角像素的颜色进行裁剪；如果alpha数据与源图像关联，则处理预乘颜色数据。</p>
   <p>设置为1可基于Alpha数据进行裁剪；corner被忽略，0总是种子值；如果没有与源图像关联的alpha数据，则不应用裁剪。</p> 
   <p><i><b>容差</b></i>  — 匹配容差。实数值0.0到1.0。指定匹配像素分量值的容差。 对于精确匹配，设置为0。</p>
   <p><i><b>infoFile</b></i>  — 要写入裁剪信息数据的XML输出文件的路径和名称。</p>

<p>  
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -embedXmpData  </span> </p> </td> 
   <td colname="col2"> <p>将XMP元数据（如果可用）从<span class="codeph"> <span class="varname"> sourceFile </span> </span>复制到<span class="codeph"> <span class="varname"> destFile </span> </span> ，无需修改。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -embedColorProfile  </span> </p> </td> 
   <td colname="col2"> <p> 在<span class="codeph"> <span class="varname"> destFile </span> </span>中嵌入ICC用户档案（如果可用）(默认情况下不嵌入用户档案)。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -imageprofile &lt;&gt; 文件 </span>&gt;  </span><span class="varname"> </span></p> </td> 
   <td colname="col2"> <p>ICC用户档案文件的路径和名称。 定义<span class="codeph"> <span class="varname"> sourceFile </span> </span>的色彩空间，并且必须与其像素类型匹配。 仅当<span class="codeph"> <span class="varname"> sourceFile </span> </span>中未嵌入用户档案时，才应指定，因为这将覆盖嵌入的用户档案。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -viewprofile  &lt;&gt; file  </span>&gt;  </span><span class="varname"> </span></p> </td> 
   <td colname="col2"> <p>ICC用户档案文件的路径和名称。 定义<span class="codeph"> <span class="varname"> destFile </span> </span>的像素类型和色彩空间。 如果<span class="codeph"> <span class="varname"> sourceFile </span> </span>具有嵌入式用户档案，或者如果还指定了<span class="codeph"> -imageprofile </span> ,IC会转换到此用户档案。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -intentRelBoricy  </span> </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>对某些颜色转换禁用黑场补偿 </p> <p>默认启动. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -cmsNoDither8  </span> </p> </td> 
   <td colname="col2"> <p>进行颜色转换时禁用仿色（错误扩散）。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -mainpailextype  </span> </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>使用标准品质（双线性）重新取样滤镜。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 如果指定为第一个选项，则在遇到无效选项时抑制使用信息的输出。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -覆盖 </span> </p> </td> 
   <td colname="col2"> <p>允许覆盖现有的<span class="codeph"> <span class="varname"> destFile </span> </span>。 默认情况下，会在文件名后附加一个数字后缀以防止覆盖。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>日志级别2及更高（默认为3000）的日志记录间隔（毫秒）。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -maxmempercent &lt;&gt; 百分 </span>比&gt;  </span><span class="varname"> </span></p> </td> 
   <td colname="col2"> <p>内存使用限制。 默认为物理内存的25%。 如果<span class="codeph"> maxmem </span>和<span class="codeph"> maxmempercent </span>均未显式设置，则使用maxmempercent默认值。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> -版本 </span> </p> </td> 
   <td colname="col2"> <p> 返回此实用程序的版本信息。 指定时没有其他选项。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_1EB2B60993D34B1887275EA4E3491401"> 
 <thead> 
  <tr> 
   ...
   <th class="entry"> <p> <b> 像</b> <b> 素TypeBits/Chan</b> </p> </th> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p> 8 </p> </td> 
   <td> <p> LZW </p> </td> 
   <td> <p> 如果存在，调板中的透明度值将转换为alpha。 </p> </td> 
  </tr> 
  <tr> 
