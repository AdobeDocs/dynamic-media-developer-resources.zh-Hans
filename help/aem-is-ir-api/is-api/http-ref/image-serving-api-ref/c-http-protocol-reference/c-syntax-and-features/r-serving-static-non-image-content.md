...
图像服务提供了一种机制，用于管理目录中的非图像内容，并通过单独的方式提供该内容 `context /is/content`. 该机制允许分别为每个项目配置TTL。
...
此机制有助于确保客户端仅接收适合其需求的内容。 假定静态内容使用适当的进行标记 `catalog::UserType`值，客户端可以添加 `type=` 命令到请求。 图像服务将提供的值与 `type=` 命令到值 `catalog::UserType` 如果不匹配，则返回错误，而不是返回可能不适当的内容。
