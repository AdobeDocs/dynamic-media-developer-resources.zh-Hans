...
指定用于视图背景的不透明填充颜色。 仅当复合图像具有透明区域或复合图像的纵横比不同于视图矩形时才可见。 忽略条件 `fmt=tif-alpha` 或 `fmt=png-alpha`，或 `req=mask`.
...
*`color`* 假定属于输出颜色空间(使用 `icc=`)并且应与输出图像具有相同的像素类型。 如果像素类型不匹配， *`color`* 使用天真的转换进行转换。
