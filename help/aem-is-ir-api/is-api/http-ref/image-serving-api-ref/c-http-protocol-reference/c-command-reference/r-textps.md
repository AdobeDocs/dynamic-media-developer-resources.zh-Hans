...
所有字体、字体颜色和段落格式控制都是使用RTF命令实现的。

参见 [文本格式](../../../../../is-api/http-ref/image-serving-api-ref/c-http-protocol-reference/c-text-formatting/c-text-formatting.md#concept-0d3136db7f6f49668274541cd4b6364c).

`textPs=` 支持扩展功能，例如对齐功能，将文本流入定义为的非矩形区域 `textFlowPath=` 和/或 `textFlowXPath=`，并沿定义的任意路径呈现文本 `textPath=`.

## 属性 {#section-a289dc26b6534b41998b1e241d5f2f92}

层属性。 应用于 `layer=0` 如果 `layer=comp`. 与互斥 `src=` 和 `text=` 在同一图层中。 最后一次出现的 `text=`， `textPs=`、和 `src=` 优先并确定这是图像还是文本图层。 被效果层忽略。
