...
平台服务器写入日志文件的文件夹。 这可以是绝对路径或相对于&#x200B;*`install_folder`*&#x200B;的路径。 默认值为&#x200B;[!DNL  *`install_folder`*/logs]。

>[!NOTE]
>
>必须在更改此设置之前创建新文件夹。 如果图像服务安装为在除根以外的用户帐户下运行，请确保文件夹具有正确的读／写访问权限。

## TC::maxDays —— 保存日志文件的天数{#section-45cbecffc5694c87b7d5c176a44a4885}

应保留日志文件的天数。 每天午夜都会创建新的日志文件。 此时，服务器将删除日志文件文件夹中早于指定天数的所有文件，包括由图像服务器或渲染服务器写入的文件。 默认值为 10。
...
指定平台服务器访问日志记录的数据模式。 模式字符串指定用其相应值替换的变量。 模式字符串中的所有其他字符都以字面方式传输到日志记录。

要使用缓存预热实用程序，必须使用空格作为字段分隔符。 平台服务器分别用`%20`和`%25`替换字段值中的所有空格和“%”字符。
...
<table id="table_7A07AFF34B5040A5B30F5735CFE9428F"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>平台服务器缓存管理关键字：<span class="codeph"> { REUSED |已创建 |已更新 |远程 | REMOTE_CREATED | REMOTE_UPDATED | REMOTE_CACHE |已验证 |已忽略 | UNDEFINED } </span>。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p><span class="codeph">标记</span>响应头值（响应数据的MD5签名）。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>与<span class="codeph"> %B </span>类似，但包括304（未修改）响应的值。 </p> </td> 
  </tr> 
  <tr> 
