---
description: 以下字段可在ICC颜色配置文件映射文件中识别。
solution: Experience Manager
title: ICC配置文件映射
feature: Dynamic Media Classic,SDK/API
role: Developer,User
exl-id: 3492f4f3-d93e-4b62-8ff6-ace228e0687e
source-git-commit: 206e4643e3926cb85b4be2189743578f88180be7
workflow-type: tm+mt
source-wordcount: '40'
ht-degree: 0%

---

# ICC配置文件映射{#icc-profile-maps}

以下字段可在ICC颜色配置文件映射文件中识别。

<table id="simpletable_91C7631EE91141DCB6EE70441BC724A9"> 
 <tr class="strow"> 
  <td class="stentry"> <p><span class="codeph"> <a href="../../../../../../is-api/image-catalog/image-serving-api-ref/c-image-catalog-reference/c-icc-profile-map-reference/r-name-icc.md#reference-9e7d3c8e35434981a3dfac66b8946cbe" type="reference" format="dita" scope="local"> 名称</a></span> </p></td> 
  <td class="stentry"> <p>颜色配置文件名称（索引键）。 </p></td> 
 </tr> 
 <tr class="strow"> 
  <td class="stentry"> <p><span class="codeph"> <a href="../../../../../../is-api/image-catalog/image-serving-api-ref/c-image-catalog-reference/c-icc-profile-map-reference/r-profilepath-icc.md#reference-d0db8b059a60437992fe1ae35761cb95" type="reference" format="dita" scope="local"> 配置文件路径</a></span> </p> </td> 
  <td class="stentry"> <p>ICC颜色配置文件路径。 </p></td> 
 </tr> 
</table>
