...
<table id="table_0AE363AB76304F258A19E43500FE8423"> 
 ...
 <tbody> 
  <tr> 
   <td> <p><b>源图像、字体、ICC配置文件</b> </p> <p> <span class="filepath"> <span class="varname"> install_folder </span>/图像 </span> <span class="codeph"></span> </p> <p> <span class="codeph"> IS::RootPaths </span> </p> </td> 
...
* *`avg_num_components`* 取决于图像类型。 对于大多数RGB图像，它为3，对于大多数CMYK或RGBA图像，它为4。 如果一半图像是RGB图像，另一半为RGBA，则使用3.5。
...
图像服务部署预期使用30,000个低分辨率旧版图像，其平均大小为500x500RGB像素。 新的打印质量图像数据预计每年将以10,000的速度添加。 典型CMYK图像大小为4k x 6k字节。 所有数据都JPEG压缩，质量高。 3年使用后的磁盘空间总量估计如下：

*`total_space`* = 30,000 x(2k + 0.5k x 0.5k x 3 x 0.1)+ 3 x 10,000 x(2k + 4k x 6k 4 x 0.1)= 2.2 G + 268 GB =约270 GB

为了获得最佳质量，可以使用deflate(zip)压缩。 假设 *`p_factor`* 在0.4中，所需的磁盘空间总量大约是4倍。 在这种情况下，略高于1 TB。
