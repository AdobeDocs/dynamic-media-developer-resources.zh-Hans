---
title: 文件柜
description: 文件柜材料指定文件柜样式文件（.vnc文件扩展名），它是一个特殊的数据文件，包含文件柜的照片表示以及参数化布局定义以及渲染文件柜正面所需的其他信息。
...
文件柜材料指定文件柜样式文件（.vnc文件扩展名），它是一个特殊的数据文件，包含文件柜的照片表示以及参数化布局定义以及渲染文件柜正面所需的其他信息。

[!DNL vnc] 文件可以包含可重复的木纹纹理，或者该纹理可以通过第二引数从外部提供 `src=`. 特定 [!DNL vnc] 文件允许对机柜前面的选定区域进行彩色化或纹理化（通常用于层压板机柜样式）。
