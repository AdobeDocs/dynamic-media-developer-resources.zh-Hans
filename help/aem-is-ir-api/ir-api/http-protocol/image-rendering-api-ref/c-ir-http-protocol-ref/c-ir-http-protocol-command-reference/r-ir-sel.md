...
在指定的像素坐标处选择组或对象 *`x, y`* 并启动新的MSS。 如果挑库位置没有可选对象，或者如果挑库位置无效，则指定的操作 `attribute::OnFailSel` 中。

*`level`* 指定是选择最外部的组，还是向下钻取到嵌套的组或对象。 如果 *`level`* 未指定，则会选择最外部的组。 设置为1可在最外部的组下选择一个组级别。 设置为大数（如99）以选择最里面的可选对象或组。

## 属性 {#section-8f27e84d88734a62a5e398e0c9972bdc}

选择命令；MSS分隔符。 对象选择将一直保留，直到选择另一个对象(使用 `obj=` 或 `sel=`.

*`x, y`* 必须在0、0（图像左上角）范围内，才能 *`wid`*-1, *`hei`*-1（图像的右下角），其中 *`wid`* 和 *`hei`* 是未缩放晕影视图的大小。
