...
<table id="simpletable_D04D41FBB03D4992B257CCBAD7EF0E7B"> 
 ...
 <tr class="strow"> 
  ...
  <td class="stentry"> <p>返回嵌入晕影中的图像映射数据。 </p> </td> 
 </tr> 
 <tr class="strow"> 
  <td class="stentry"> <p> <span class="codeph"> object </span> </p> </td> 
  <td class="stentry"> <p>运行命令，并将遮罩的渲染图像返回到当前对象选择。 </p> </td> 
 </tr> 
 <tr class="strow"> 
...
除非在详细说明中另有说明，否则服务器返回MIME类型为&lt;text/plain>的文本响应。

`debug`

执行指定的命令并返回渲染的图像。 如果发生错误，则返回错误和调试信息，而不是错误图像(`attribute::ErrorImagePath`)。
...
<table id="table_A30296D29B5D43F1B5383A887252C6B4"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col3"> <p> <span class="codeph"> attribute::Expiration </span> 或默认的有效时间。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col3"> <p>1，如果关联用户档案嵌入晕影。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> image.embedded PhotoshopPaths  </span> </p> </td> 
   <td colname="col2"> <p>布尔值 </p> </td> 
   <td colname="col3"> <p>1如果晕影嵌入了路径数据。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> image.modifier  </span> </p> </td> 
   <td colname="col2"> <p>字符串型 </p> </td> 
   <td colname="col3"> <p> <span class="codeph"> attribute::Modifier </span> 或空（如果不是目录条目）。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>字符串型 </p> </td> 
   <td colname="col3"> <p>修改日期/时间（来自<span class="codeph">目录：:TimeStamp </span>或晕影文件）。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Real </p> </td> 
   <td colname="col3"> <p>最大对象分辨率，以<a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-syntax-and-features/c-ir-vignettes/c-ir-material-resolution.md#concept-f60103c64e324e2cae78bd76dfb4de8b" type="concept" format="dita" scope="local">材质分辨率</a>单位（通常为像素/英寸）表示。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> vignette.res.avg  </span> </p> </td> 
   <td colname="col2"> <p>Real </p> </td> 
   <td colname="col3"> <p>以<a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-syntax-and-features/c-ir-vignettes/c-ir-material-resolution.md#concept-f60103c64e324e2cae78bd76dfb4de8b" type="concept" format="dita" scope="local">材质分辨率</a>单位（通常为像素/inc <a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-syntax-and-features/c-ir-vignettes/c-ir-material-resolution.md#concept-f60103c64e324e2cae78bd76dfb4de8b" type="concept" format="dita" scope="local">材质分辨率</a>h）表示的对象平均分辨率。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> vignette.res.min  </span> </p> </td> 
   <td colname="col2"> <p>Real </p> </td> 
   <td colname="col3"> <p>以<a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-syntax-and-features/c-ir-vignettes/c-ir-material-resolution.md#concept-f60103c64e324e2cae78bd76dfb4de8b" type="concept" format="dita" scope="local">材质分辨率</a>单位（通常为像素/英寸）表示的最小对象分辨率。 </p> </td> 
  </tr> 
  <tr> 
...
返回晕影中包含的图像映射数据。 默认情况下，返回所有最外面的组的映射数据。 可以使用

`req=map&groupLevel=-1`

映射数据不会缩放到`wid=`或`hei=`，或以其他方式修改。 响应MIME类型为`<text/xml>`。

响应数据由`<map>`元素组成，该元素包含一组`<area>`元素，与HTML `<AREA>`标签类似。

每个`<area>`元素都包括标准`type=`和`coord=`属性，以及指定晕影组名称或名称路径的`name=`属性。 如果相应对象组的蒙版具有不连续区域，则存在多个同名`<area>`元素。
...
如果晕影不包含映射文档，则返回一个XML，其中`<map>`元素为空。

`object`

执行指定的命令并返回由剩余对象选择（使用请求中最后一个`sel=`或`obj=`命令选择的组或对象）所遮罩的渲染图像。 通常与支持alpha的图像格式一起使用（请参阅[fmt=](../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-fmt.md#reference-4c743f67d56b47c5b774fcc900ff758c)）。 如果使用的图像格式不支持Alpha，则蒙版外部的区域为黑色。

`props`

执行指定的命令并返回晕影属性和组或对象属性，而不是渲染的图像。 有关回复语法和响应MIME类型的说明，请参阅[属性](../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-response-data/c-ir-properties.md#concept-e99f1a373eae4af9b41842ca0088ad3a)。 除非也指定了`obj=`或`sel=`，否则将应用默认选择（请参阅[ `obj=` ](../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-obj.md#reference-31e7dac7931b4e0eb3c7589f120a1e6a)）。
...
<table id="table_F3ECF0584F6247A2A75C1CAFE1C57A4E"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p> 布尔值 </p> </td> 
   <td> <p>如果ICC用户档案将嵌入到回复图像中，则为true（请参阅<span class="codeph"> <a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-iccembed.md#reference-47a433138c7c4b29b9b29871b2491a7f" type="reference" format="dita" scope="local"> iccEmbed= </a> </span>）。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>整数，布尔值 </p> </td> 
   <td> <p> JPEG品质和色度标志（请参阅<span class="codeph"> <a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-qlt.md#reference-27b91c226eb241d0a14a29af3b3afdbd" type="reference" format="dita" scope="local"> qlt= </a> </span>） </p> </td> 
  </tr> 
  <tr> 
...
返回`vignette::UserData`的内容。 服务器将用行终结器(`<cr><lf>`)替换`vignette::UserData`中所有出现的`'??'`。 回复格式化为文本数据，响应MIME类型设置为&lt;text/plain>。

如果在URL路径中指定的对象未解析为有效的晕影映射条目，或者`vignette::UserData`为空，则回复将仅包含行终结器(`CR/LF`)。
