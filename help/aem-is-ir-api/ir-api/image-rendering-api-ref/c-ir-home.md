---
title: 影像演算API
description: 影像轉譯API參考資料包含有關設定和使用Dynamic Media影像轉譯API的資訊。
solution: Experience Manager
feature: Dynamic Media Classic,SDK/API
role: Developer,User
exl-id: e7cf4b21-8020-436a-b084-b81e9c8a8b9b
source-git-commit: 8454991568374ecd1c4babdd3210250ea7988c4c
workflow-type: tm+mt
source-wordcount: '63'
ht-degree: 1%

---

# 影像演算API{#image-rendering-api}

影像轉譯API參考資料包含有關設定和使用Dynamic Media影像轉譯API的資訊。

* [配置](c-ir-installation/c-ir-installation.md)
* [授權](c-ir-installation/c-ir-licensing.md)
* [設定和設定IR 3.x相容性模組](c-ir-installation/t-ir-setup-compatibility-module.md)
* [設定和設定影像演算](c-ir-installation/c-ir-setup-ir.md)
* [解除安裝影像演算](c-ir-installation/c-ir-uninstalling/c-ir-uninstalling.md)
* [伺服器管理](c-ir-server-administration/c-ir-server-administration.md)
