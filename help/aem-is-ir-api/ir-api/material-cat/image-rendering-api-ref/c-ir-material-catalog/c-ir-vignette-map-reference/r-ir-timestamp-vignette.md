...
如果 `attribute::UseLastModified` 设置为，则HTTP响 `vignette::TimeStamp` 应中 `catalog::TimeStamp`会作为上次修改的头返回暗角的最近值和值以及请求中涉及的所有材料。
