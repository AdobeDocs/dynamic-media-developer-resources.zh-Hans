...
借助基于过期的验证，会定期检查源材料和晕影，以查看它们是否已更改。 使用基于目录的验证，源图像仅在值更改后 `catalog::TimeStamp` 才被检查。

在同时使用材料目录和暗角目录时，建议使用基于目录的验证。 当晕影在图像渲染请求中直接按路径引用时，应使用基于过期的验证。

## 属性 {#section-46e13cb341eb442c86e0d8292de23ea0}

枚举。 0，选择基于过期的验证。 1，选择基于目录的缓存验证。
