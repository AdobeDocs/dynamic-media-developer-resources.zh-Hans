---
description: 选取选择错误处理。 指定sel=命令由于指定的像素位置不在可选对象的遮罩区域内而失败时要执行的操作。
seo-description: 选取选择错误处理。 指定sel=命令由于指定的像素位置不在可选对象的遮罩区域内而失败时要执行的操作。
seo-title: OnFailSel
...
选取选择错误处理。 指定sel=命令由于指定的像素位置不在可选对象的遮罩区域内而失败时要执行的操作。
