---
description: 默认客户端缓存的有效时间。 提供默认的过期间隔，以防特定目录记录不包含有效的目录过期值或晕影到期值，或者如果直接访问晕影文件或材料文件，而不是通过目录记录访问。
seo-description: 默认客户端缓存的有效时间。 提供默认的过期间隔，以防特定目录记录不包含有效的目录过期值或晕影到期值，或者如果直接访问晕影文件或材料文件，而不是通过目录记录访问。
seo-title: 過期
...
默认客户端缓存的有效时间。 提供默认的过期间隔，以防特定目录记录不包含有效的目录：：到期或暗角：：到期值，或者如果直接访问暗角文件或材料文件，而不是通过目录记录访问。

## 属性 {#section-8e2bade105ec4905ae5c4911f500279f}

实数，0或更大。 自生成回复数据以来到期的小时数。 设置为0可始终立即使回复图像过期，这会有效地禁用客户端缓存。 设置为-1可标记为永不 *过期*。
