---
title: 缩小按钮
description: 选择此按钮将会在主视图中放大图像。 此按钮不会显示在手机上以节省屏幕资产。 您可以使用CSS调整按钮的大小、外观和位置。
...
选择此按钮将会在主视图中放大图像。 此按钮不会显示在手机上以节省屏幕资产。 您可以使用CSS调整按钮的大小、外观和位置。
...
示例 — 设置一个28 x 28像素的缩小按钮，该按钮距底部4像素，距主控制栏右边缘75像素。 最后，针对四个不同的按钮状态中的每个状态显示一个不同的图像。
