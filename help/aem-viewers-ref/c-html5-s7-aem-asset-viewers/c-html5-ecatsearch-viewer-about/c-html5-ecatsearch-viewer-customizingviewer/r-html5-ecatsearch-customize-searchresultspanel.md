...
当面板处于活动状态时，查看器用户界面将覆盖半透明填充。 此填充的颜色和不透明度可通过以下CSS类选择器进行控制：
...
示例 — 要在大中型断点上设置一个250像素宽的搜索结果面板，并在小型断点上使用全尺寸面板：
...
示例 — 设置一个26 x 26像素的搜索按钮，用于“查看玻璃”图标；大小为30像素，边框为1像素：
...
当首次调用该特征时，搜索结果面板可能显示文本提示。 当用户搜索未返回任何结果时，也会向用户显示一条消息。 在所有情况下，文本都会显示在搜索结果面板的主要部分中，并由以下CSS类选择器控制：
...
>[!NOTE]
>
>此文本面板支持`state`属性选择器，该选择器可用于将不同的样式应用于不同的文本消息。 特别是， `state='prompt'`对应于首次调用面板时显示的文本提示；`state='results'`对应于包含搜索点击信息的文本；和`state='no_results'`对应于搜索查询未返回任何结果时显示的文本。

消息文本可以本地化。 有关更多信息，请参阅[用户界面元素的本地化](../../../c-html5-s7-aem-asset-viewers/c-html5-ecatsearch-viewer-about/c-html5-ecatsearch-viewer-localization.md#concept-cbfc39344c494eb7b9f6a272cff0cc74) 。
...
示例 — 要设置缩略图215 x 129像素，缩略图的默认边框为浅灰色，选定的边框为深灰色：
...
>[!NOTE]
>
>此按钮支持`state`属性选择器，该选择器可用于对`"up"`、`"down"`、`"over"`和`"disabled"`按钮状态应用不同的外观。
