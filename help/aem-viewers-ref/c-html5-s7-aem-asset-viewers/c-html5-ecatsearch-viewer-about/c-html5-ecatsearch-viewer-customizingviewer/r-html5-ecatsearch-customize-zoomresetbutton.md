---
title: 缩放重置按钮
description: 选择此按钮可重置主视图中的图像。 此按钮显示在桌面系统和平板电脑的主控制栏中。 在手机上，此按钮显示在图像底部的正上方。 但是，当图像处于重置状态时，不会显示它。 您可以使用CSS调整此按钮的大小、外观和位置。
...
选择此按钮可重置主视图中的图像。 此按钮显示在桌面系统和平板电脑的主控制栏中。 在手机上，此按钮显示在图像底部的正上方。 但是，当图像处于重置状态时，不会显示它。 您可以使用CSS调整此按钮的大小、外观和位置。
...
示例 — 设置一个28 x 28像素的缩放重置按钮，该按钮位于桌面上，距主控栏的底部4像素，距右边缘47像素。 最后，为四种不同的按钮状态中的每种状态显示不同的图像。
