...
>[!NOTE]
>
>此按钮支持`state`和`selected`属性选择器，这两个选择器可用于将不同的外观应用于不同的按钮状态。
>
>特别地，`selected='false'`对应于初始滚动按钮状态，而`selected='true'`对应于当搜索面板处于活动状态时的状态。
...
示例 — 设置一个28 x 28像素的“搜索”按钮，并在选中或未选中时，为四个不同按钮状态中的每个状态显示不同的图像。
