---
title: “关闭”按钮
description: 单击或点按此按钮可关闭包含的网页。 仅当closebutton参数设置为1时，才会显示此按钮。 此按钮在桌面系统上不可用。 您可以使用CSS调整按钮的大小、外观和位置。
...
选择或点按此按钮可关闭包含的网页。 仅当closebutton参数设置为1时，才会显示此按钮。 此按钮在桌面系统上不可用。 您可以使用CSS调整按钮的大小、外观和位置。
...
示例 — 设置一个56 x 56像素的“关闭”按钮，该按钮距主控制栏的上边缘和右边缘4像素。 最后，针对四个不同的按钮状态中的每个状态显示一个不同的图像。
