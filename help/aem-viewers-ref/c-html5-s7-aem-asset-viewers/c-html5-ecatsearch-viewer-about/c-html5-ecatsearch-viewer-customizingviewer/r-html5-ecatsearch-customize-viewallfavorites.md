...
>[!NOTE]
>
>此按钮同时支持 `state` 和 `selected` 属性选择器，可用于将不同的外观应用于不同的按钮状态。 特别是， `selected='true'` 对应于用户通过单击或点按可添加新的“收藏”图标的状态。 此 `selected='false'` 对应于用户可以缩放、平移和交换页面的正常操作模式。
