---
title: 目录
description: 目錄是主控制列中的按鈕。 啟動後，下拉式面板隨即顯示，內含頁面索引和標籤清單。
...
目錄是主控制列中的按鈕。 啟動後，下拉式面板隨即顯示，內含頁面索引和標籤清單。
...
範例 — 設定目錄按鈕，其位置從底部算起4畫素，從主控制列左側算起43畫素。 大小為28 x 28畫素，而且會針對四種不同的按鈕狀態分別顯示不同的影像：
...
如果下拉式面板中的專案數量超過垂直容納量（且系統為桌上型電腦），元件會在面板右側呈現垂直卷軸。 卷軸區域的外觀是由下列CSS類別選取器所控制：
