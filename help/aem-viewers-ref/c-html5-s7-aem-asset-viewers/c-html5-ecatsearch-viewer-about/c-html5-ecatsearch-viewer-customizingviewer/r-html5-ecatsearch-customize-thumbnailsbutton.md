---
title: “缩略图”按钮
description: 选择此按钮将重置在主视图和缩略图之间切换查看器。 此按钮显示在主控制栏中。 您可以使用CSS调整按钮的大小、外观和位置。
...
选择此按钮将重置在主视图和缩略图之间切换查看器。 此按钮显示在主控制栏中。 您可以使用CSS调整按钮的大小、外观和位置。
...
>[!NOTE]
>
>此按钮支持 `state` 和 `selected` 属性选择器，可用于将不同的外观应用于不同的按钮状态。 特别是， `selected='true'` 与缩略图模式处于活动状态时的查看器状态相对应，并且 `selected='false'` 对应于主视图的默认状态。
...
示例 — 设置一个28 x 28像素的缩略图按钮，该按钮距底部4像素，距主控制栏左边缘5像素。 最后，在选择或未选择时，针对四个不同的按钮状态中的每个状态显示一个不同的图像。
