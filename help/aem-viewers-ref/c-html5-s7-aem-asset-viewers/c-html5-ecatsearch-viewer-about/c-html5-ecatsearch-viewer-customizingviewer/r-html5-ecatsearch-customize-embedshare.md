...
示例 — 要设置带内边距的对话框标题，请设置24 x 14像素图标和粗体16点标题。 最后，一个28 x 28像素的“关闭”按钮，从顶部放置两个像素，从对话框容器右侧放置两个像素：
...
>[!NOTE]
>
>组合框支持 `expanded` 属性选择器，可能的值为 `true` 和 `false`. 值 `true` 在组合框显示预定义的嵌入大小之一时使用，因此应采用所有可用宽度。 值 `false` 在组合框中选择了“自定义大小”选项时使用，因此应将其收缩以便为自定义宽度和高度输入字段留出空间。
