...
创建自定义CSS时，请记住查看器分配 `.s7ecatalogsearchviewer` 类添加到其容器DOM元素。 如果您使用的是与 `style=` 命令，使用 `.s7ecatalogsearchviewer` 类作为CSS规则的子选择器中的父类。 如果您在网页上执行嵌入的样式，则还应使用容器DOM元素的ID来限定此选择器，如下所示：

`#<containerId>.s7ecatalogsearchviewer`

## 构建响应式设计的CSS {#section-c1e74f5114ad418884ca1c95f5ea5b63}

可以定位CSS中的不同设备和嵌入大小，以使内容显示方式不同，具体取决于用户的设备或特定网页布局。 此定位包括但不限于不同的网页布局、用户界面元素大小和图稿分辨率。

查看器支持两种创建响应式设计CSS的方法：CSS标记和标准CSS媒体查询。 您可以单独或结合使用这些方法。

**CSS标记**

要创建响应式设计的CSS，查看器支持CSS标记，这些CSS标记根据运行时查看器大小和当前设备上的输入类型，动态地分配给顶级查看器容器元素的特殊CSS类。

第一组CSS标记包括 `.s7size_large`, `.s7size_medium`和 `.s7size_small` 类。 它们基于查看器容器的运行时区域应用。 即，如果查看器区域等于或大于公共桌面显示器的大小 `.s7size_large` ，则如果区域的大小接近普通平板电脑设备 `.s7size_medium` 分配。 适用于类似于手机屏幕的区域。 标记 `.s7size_small` 设置。 这些CSS标记的主要用途是为不同屏幕和查看器大小创建不同的用户界面布局。
...
以下示例CSS可将带有鼠标输入的系统上的按钮放大大小设置为28 x 28像素，将触控设备上的按钮放大大小设置为56 x 56像素。 此外，如果查看器大小过小，它会完全隐藏按钮：
...
使用CSS标记是构建响应式设计的CSS的最灵活方法。 它不仅允许您定位设备屏幕大小，还允许您定位实际查看器大小，这对于响应式设计的页面布局非常有用。
