...
description: eCatalog搜索查看器的所有可视化自定义和大多数行为自定义均通过创建自定义CSS来完成。
keywords: 响应式
...
可以在CSS中定位不同的设备和嵌入大小，以使内容的显示有所不同，具体取决于用户的设备或特定网页布局。 此定位包括但不限于不同的网页布局、用户界面元素大小和图稿分辨率。
...
使用CSS标记是构建响应式设计CSS的最灵活方法。 它不仅允许您定位设备屏幕大小，而且允许您定位实际查看器大小，这对于设计响应式页面布局非常有用。
...
* [大下一页按钮](r-html5-ecatsearch-customize-largenextpagebutton.md)
