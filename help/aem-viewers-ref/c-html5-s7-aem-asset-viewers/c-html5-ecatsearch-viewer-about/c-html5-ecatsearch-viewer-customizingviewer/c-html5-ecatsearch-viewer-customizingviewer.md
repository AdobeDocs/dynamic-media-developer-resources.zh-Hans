---
description: 电子目录搜索查看器的所有可视自定义和大多数行为自定义都是通过创建自定义CSS完成的。
keywords: responsive
seo-description: 电子目录搜索查看器的所有可视自定义和大多数行为自定义都是通过创建自定义CSS完成的。
...
电子目录搜索查看器的所有可视自定义和大多数行为自定义都是通过创建自定义CSS完成的。
...
创建自定义CSS时，请牢记查看器将`.s7ecatalogsearchviewer`类分配给其容器DOM元素。 如果使用通过`style=`命令传递的外部CSS文件，请在CSS规则的子代选择器中使用`.s7ecatalogsearchviewer`类作为父类。 如果要在网页上执行嵌入样式，则还可以按如下方式将此选择器与容器DOM元素的ID一起限定：
