...
>[!NOTE]
>
>此按钮同时支持`state`和`selected`属性选择器，它们可用于将不同的外观应用于不同的按钮状态。 特别是，`selected='true'`对应于用户通过选择或点按可添加新“收藏”图标的状态。 而`selected='false'`对应于用户可以缩放、平移和交换页面的正常操作模式。
...
示例 — 设置一个28 x 28像素的“添加收藏夹”按钮，并在选中或未选中时，为四种不同的按钮状态中的每种状态显示不同的图像。
