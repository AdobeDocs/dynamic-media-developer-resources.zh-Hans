---
description: 电子目录搜索查看器是一个目录查看器，它按跨页或逐页显示电子小册子。电子目录允许用户使用其他用户界面元素或专用缩略图模式在目录中导航。 用户还可以放大每个页面，以获得更详细的信息。
keywords: responsive
seo-description: 电子目录搜索查看器是一个目录查看器，它按跨页或逐页显示电子小册子。电子目录允许用户使用其他用户界面元素或专用缩略图模式在目录中导航。 用户还可以放大每个页面，以获得更详细的信息。
seo-title: 电子目录搜索
...
电子目录搜索查看器是一个目录查看器，它按跨页或逐页显示电子小册子。电子目录允许用户使用其他用户界面元素或专用缩略图模式在目录中导航。 用户还可以放大每个页面，以获得更详细的信息。
...
eCatalog Search Viewer表示主JavaScript文件和一组帮助文件(单个JavaScript包含该查看器的所有查看器SDK组件，该查看器由查看器在运行时下载

您可以使用随IS-Viewers提供的生产就绪型HTML页面，在弹出模式下使用eCatalog Search Viewer，或在嵌入式模式下使用文档化的API将其集成到目标网页中。
...
eCatalog搜索查看器支持社交共享工具。这些工具可作为主控件栏中的按钮使用，当用户单击或点按时，该按钮会扩展到共享工具栏中。
...
在主工具栏中，查看器的“搜索”功能以外观玻璃图标的形式提供。 单击或点按图标可激活带有输入字段的“搜索”面板。 在输入关键字或短语并按Enter后，查看器将在面板中呈现搜索结果并高亮显示主视图中找到的单词。
...
建议您使用现成的HTML页面进行弹出操作模式。 在这种情况下，它被调用， [!DNL eCatalogSearchViewer.html] 并位于标准IS查看 [!DNL html5/] 器部署的子文件夹中：
...
1. 将查看器JavaScript文件添加到网页。
   ...
   创建查看器需要在HTML头中添加脚本标记。 在使用查看器API之前，请确保包含该查看器 [!DNL eCatalogSearchViewer.js]。 该文 [!DNL eCatalogSearchViewer.js] 件位于标准IS查看 [!DNL html5/js/] 器部署的子文件夹下：
...
1. 设置查看器大小
   ...
   可以通过以绝对单位声明顶级CSS类的查看器，或 `.s7ecatalogsearchviewer` 者使用修饰符来设置查看器的静态 `stagesize` 大小。
...
1. 初始化查看器。
   ...
   完成上述步骤后，您将创建类的一个实例，将所有配置信息传递给它的构造函数，并对查看器实例 `s7viewers.eCatalogSearchViewer``init()` 调用方法。 配置信息作为JSON对象传递给构造函数。 此对象至少包含一个字段， `containerId` 该字段包含查看器容器ID的名称以及查看器支持的配置参数的嵌套 `params` JSON对象。 在这种情况下，对 `params` 象必须至少将图像服务URL作为属性传递， `serverUrl` 并将初始资产作为参 `asset` 数。 基于JSON的初始化API允许您使用单行代码创建和开始查看器。
   ...
   以下是创建查看器实例的示例，将最小必要的配置选项传递给构造函数并调用方 `init()` 法。 示例假定 `eCatalogSearchViewer` 是查看器实例；是 `s7viewer` 占位符的名称 `DIV`;是图 `http://s7d1.scene7.com/is/image/` 像服务URL, `Viewers/Pluralist` 也是资产：
   ...
   以下代码是一个简单网页的完整示例，该网页嵌入了具有固定大小的eCatalog Search Viewer:
