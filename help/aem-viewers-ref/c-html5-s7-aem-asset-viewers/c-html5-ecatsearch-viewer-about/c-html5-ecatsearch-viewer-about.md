...
1. 初始化查看器。
   ...
   以下示例用于创建查看器实例，将最小必需的配置选项传递给构造函数，并调用`init()`方法。 该示例假定`eCatalogSearchViewer`为查看器实例；`s7viewer`是占位符`DIV`的名称；`https://s7d1.scene7.com/is/image/`是图像服务URL，`Viewers/Pluralist`是资产：
