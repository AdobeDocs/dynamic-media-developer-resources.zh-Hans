---
description: eCatalog Search Viewer是一个目录查看器，它以逐页或逐页的方式显示电子小册子。通过eCatalog，用户可以使用其他用户界面元素或专用缩略图模式浏览目录。 用户还可以放大每个页面，获得更详细的信息。
keywords: 响应
...
eCatalog Search Viewer是一个目录查看器，它以逐页或逐页的方式显示电子小册子。通过eCatalog，用户可以使用其他用户界面元素或专用缩略图模式浏览目录。 用户还可以放大每个页面，获得更详细的信息。
...
eCatalog Search Viewer表示主JavaScript文件和一组帮助文件（单个JavaScript包含在由查看器在运行时下载的由此特定查看器、资源、CSS使用的所有查看器SDK组件中）

您可以使用随IS-Viewer提供的生产就绪型HTML页面，在弹出模式下使用eCatalog Search Viewer，或在嵌入模式下使用文档化API将其集成到目标网页中。
...
## 与eCatalog Search Viewer {#section-642e66ca38cd4032992840ec6c0b0cd2}交互

eCatalog Search Viewer支持其他移动应用程序中常见的以下触控手势。
...
## 使用eCatalog Search Viewer {#section-eb575084a99647c3a9591f439f40b412}的社交媒体共享工具

eCatalog搜索查看器支持社交共享工具。这些工具可用作主控件栏中的按钮，当用户单击或点按时，该按钮将扩展到共享工具栏。
...
查看器的“搜索”功能在主工具栏中以外观玻璃图标的形式提供。 单击或点按该图标将激活带输入字段的“搜索”面板。 在输入关键字或短语并按Enter后，查看器将在面板中呈现搜索结果并高亮显示主视图中找到的单词。
...
建议您使用现成的HTML页面进行弹出操作模式。 在这种情况下，它称为[!DNL eCatalogSearchViewer.html]，位于标准IS-Viewers部署的[!DNL html5/]子文件夹中：
...
1. 将查看器JavaScript文件添加到网页。
   ...
   创建查看器需要在HTML头中添加脚本标签。 在使用查看器API之前，请确保包含[!DNL eCatalogSearchViewer.js]。 [!DNL eCatalogSearchViewer.js]文件位于标准IS-Viewers部署的[!DNL html5/js/]子文件夹下：
...
1. 设置查看器大小
   ...
   您可以通过以下两种方式设置查看器的静态大小：以绝对单位为`.s7ecatalogsearchviewer`顶级CSS类声明查看器，或使用`stagesize`修饰符。
...
1. 正在初始化查看器。
   ...
   完成上述步骤后，您将创建`s7viewers.eCatalogSearchViewer`类的实例，将所有配置信息传递给它的构造函数，并对查看器实例调用`init()`方法。 配置信息作为JSON对象传递给构造函数。 此对象至少具有`containerId`字段，该字段包含查看器容器ID的名称以及查看器支持的配置参数嵌套的`params` JSON对象。 在这种情况下，`params`对象必须至少具有作为`serverUrl`属性传递的图像服务URL，以及作为`asset`参数传递的初始资产。 基于JSON的初始化API允许您使用单行代码创建和开始查看器。
   ...
   以下是创建查看器实例、将最少的必要配置选项传递给构造函数并调用`init()`方法的示例。 该示例假定`eCatalogSearchViewer`为查看器实例；`s7viewer`是占位符`DIV`的名称；`http://s7d1.scene7.com/is/image/`是图像服务URL，`Viewers/Pluralist`是资产：
   ...
   以下代码是嵌入具有固定大小的eCatalog搜索查看器的次要网页的完整示例：
