...
查看器用于加载和初始化SDK组件的命名空间取决于查看器运行的环境。 如果查看器在AEM (Adobe Experience Manager)中运行，查看器会将SDK组件加载到`s7viewers.s7sdk`命名空间中。 Dynamic Media Classic提供的查看器会将SDK加载到`s7classic.s7sdk`中。
