...
建议您使用现成的HTML页面进行弹出操作模式。 在这种情况下，它称为 [!DNL SpinViewer.html] 和位于 [!DNL html5/] 标准IS — 查看器部署的子文件夹：
...
1. 将查看器JavaScript文件添加到网页。
   ...
   创建查看器要求您在HTML头中添加脚本标记。 在使用查看器API之前，请确保在 `SpinViewer.js`. `SpinViewer.js` 位于 [!DNL html5/js/] 标准IS — 查看器部署的子文件夹：
