...
查看器提供了用于“浅”和“深”颜色方案的现成CSS文件。 默认使用“浅色”版本，但您可以使用以下标准CSS切换到“深色”版本：
...
创建自定义CSS时，请记住，查看器将`.s7spinviewer`类分配给其容器DOM元素。 如果您使用通过`style=`命令传递的外部CSS文件，请将`.s7spinviewer`类用作CSS规则的后代选择器中的父类。 如果您在网页上执行嵌入样式，则还可以使用容器DOM元素的ID限定此选择器，如下所示：
