---
description: 打开和关闭隐藏式字幕显示。 如果未指定题注参数，则不可见。 您可以使用CSS相对于包含该按钮的控件栏来调整其大小、设置其外观和定位。
seo-description: 打开和关闭隐藏式字幕显示。 如果未指定题注参数，则不可见。 您可以使用CSS相对于包含该按钮的控件栏来调整其大小、设置其外观和定位。
seo-title: “题注”按钮
...
打开和关闭隐藏式字幕显示。 如果未指定题注参数，则不可见。 您可以使用CSS相对于包含该按钮的控件栏来调整其大小、设置其外观和定位。
...
>[!NOTE]
>
>此按钮支持`state`属性选择器和选定的属性选择器，这些选择器可用于将不同的外观应用于不同的按钮状态。 尤其是，`selected='true'`对应于字幕可见时的状态，而`selected='false'`则用于隐藏字幕时的状态。
...
示例——设置一个28 x 28像素的隐藏式字幕按钮，从控件条的顶部放置4个像素，从右边缘放置68个像素，在选择或未选择时，为四个不同按钮状态中的每个状态显示不同的图像。
