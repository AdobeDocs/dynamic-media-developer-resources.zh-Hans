---
title: 题注按钮
description: 打开和关闭隐藏式字幕显示。 如果未指定描述参数，则不会显示标题。 可以使用CSS调整此按钮的大小、外观以及相对于包含此按钮的控制栏的定位。
...
打开和关闭隐藏式字幕显示。 如果未指定描述参数，则不会显示标题。 可以使用CSS调整此按钮的大小、外观以及相对于包含此按钮的控制栏的定位。
...
>[!NOTE]
>
>此按钮支持`state`属性选择器和选定的属性选择器，它们可用于将不同的外观应用于不同的按钮状态。 特别是，`selected='true'`对应于字幕可见时的状态，而隐藏字幕时使用`selected='false'`。
...
示例 — 设置一个“隐藏式字幕”按钮，该按钮为28 x 28像素，从顶部放置4像素，从控制栏的右边缘放置68像素。 最后，在选中或未选中时，为四种不同的按钮状态中的每种状态显示不同的图像。
