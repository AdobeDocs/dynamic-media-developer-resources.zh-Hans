---
description: 打开和关闭隐藏式字幕显示。 如果未指定题注参数，则不显示该标题。 您可以使用CSS来相对于包含该按钮的控件栏来调整该按钮的大小、外观和位置。
seo-description: 打开和关闭隐藏式字幕显示。 如果未指定题注参数，则不显示该标题。 您可以使用CSS来相对于包含该按钮的控件栏来调整该按钮的大小、外观和位置。
seo-title: “题注”按钮
...
打开和关闭隐藏式字幕显示。 如果未指定题注参数，则不显示该标题。 您可以使用CSS来相对于包含该按钮的控件栏来调整该按钮的大小、外观和位置。
...
>[!NOTE]
>
>此按钮支持属 `state` 性选择器和选定的属性选择器，这些选择器可用于将不同的外观应用于不同的按钮状态。 特别是，与 `selected='true'` 字幕可见时的状态对应，并在字 `selected='false'` 幕隐藏时使用。
...
示例——设置一个28 x 28像素的隐藏式字幕按钮，从控件栏的顶部放置四个像素，从右边缘放置68像素，在选择或不选择时，为四个不同按钮状态中的每个状态显示不同的图像。
