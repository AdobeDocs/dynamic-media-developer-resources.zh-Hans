---
description: 打开和关闭隐藏式字幕显示。 如果未指定题注参数，则不可见。 您可以使用CSS来调整此按钮的大小、外观和相对于包含该按钮的控件栏定位。
...
打开和关闭隐藏式字幕显示。 如果未指定题注参数，则不可见。 您可以使用CSS来调整此按钮的大小、外观和相对于包含该按钮的控件栏定位。
...
>[!NOTE]
>
>此按钮支持`state`属性选择器和选定的属性选择器，这些选择器可用于将不同的外观应用于不同的按钮状态。 具体而言，`selected='true'`对应于字幕可见时的状态，而隐藏字幕时使用`selected='false'`。
...
示例 — 设置一个28 x 28像素的隐藏式字幕按钮，从控件条的顶部放置4个像素，从右边缘放置68个像素，在选中或未选中时，为四个不同按钮状态中的每个状态显示不同的图像。
