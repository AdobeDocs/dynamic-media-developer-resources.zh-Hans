---
title: 题注按钮
description: 打开和关闭隐藏式字幕显示。 如果未指定题注参数，则不会显示此字段。 可以使用CSS调整此按钮的大小、外观以及相对于包含它的控制栏的定位。
...
打开和关闭隐藏式字幕显示。 如果未指定题注参数，则不会显示此字段。 可以使用CSS调整此按钮的大小、外观以及相对于包含它的控制栏的定位。
...
>[!NOTE]
>
>此按钮支持 `state` 属性选择器和选定的属性选择器，可用于将不同的外观应用于不同的按钮状态。 特别是， `selected='true'` 对应于字幕可见时的状态，并且 `selected='false'` 隐藏字幕时使用。
...
示例 — 设置一个隐藏式字幕按钮，该按钮为28 x 28像素，位于距离顶部4个像素的位置，距离控制栏右边缘68个像素。 最后，在选中或未选中时，为四个不同的按钮状态中的每一个显示不同的图像。
