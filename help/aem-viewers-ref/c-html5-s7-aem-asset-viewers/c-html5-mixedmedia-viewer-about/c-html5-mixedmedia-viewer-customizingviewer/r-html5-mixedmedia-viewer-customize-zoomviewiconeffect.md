---
description: 缩放指示符被覆盖在缩放视图区上。 当图像处于重置状态时，它会显示，并且它还取决于图标效果参数。
seo-description: 缩放指示符被覆盖在缩放视图区上。 当图像处于重置状态时，它会显示，并且它还取决于图标效果参数。
seo-title: 缩放视图图标效果
...
缩放指示符被覆盖在缩放视图区上。 当图像处于重置状态时，它会显示，并且它还取决于图标效果参数。
...
>[!NOTE]
>
>图标效果支持属 `media-type` 性选择器，您可以使用它在不同设备上应用不同的图标效果。 特别地，对 `media-type='standard'` 应于桌面系统，其中鼠标输入通常被使用，并对 `media-type='multitouch'` 应于具有触摸输入的设备。
