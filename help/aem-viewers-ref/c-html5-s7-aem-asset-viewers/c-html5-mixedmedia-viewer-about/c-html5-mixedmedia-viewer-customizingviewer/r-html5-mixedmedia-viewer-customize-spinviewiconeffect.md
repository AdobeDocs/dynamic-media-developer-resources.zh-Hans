---
title: 旋转视图图标效果
description: 旋转指示器覆盖在旋转视图区域上。 当图像处于重置状态时显示，并且还取决于iconeffect参数。
...
旋转指示器覆盖在旋转视图区域上。 当图像处于重置状态时显示，并且还取决于iconeffect参数。
...
旋转指示器支持 `state` 属性选择器，设置为 `spin_1D` 如果存在一维旋转集，并且 `spin_2D` 如果存在多维旋转集。
