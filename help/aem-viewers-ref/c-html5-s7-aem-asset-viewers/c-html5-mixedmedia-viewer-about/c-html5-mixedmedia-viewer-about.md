...
查看器底部的缩略图表示每个媒体集元素及其资产类型指示符。 选择样本集元素后，将显示第二行样本以允许选择样本集中的颜色变化。 图像和样本集元素支持连续或内联模式的缩放；旋转集支持缩放和旋转。 视频和自适应视频集支持所有基本的播放控件，只要视频内容顶部显示任何可选的隐藏式字幕即可。 用户可以通过单击全屏按钮随时切换到全屏。 查看器具有可选的关闭按钮。 它设计为可在桌面和移动设备上使用。

当基础系统支持HLS格式时，混合媒体查看器将HTML5流视频播放作为其默认配置。 在不支持HTML5流的系统上，查看器返回到HTML5渐进式视频投放。
...
混合媒体查看器表示主JavaScript文件和一组帮助文件（单个JavaScript包含该查看器在运行时下载的特定查看器、资源、CSS所使用的所有查看器SDK组件）。

您可以使用随IS查看器提供的生产就绪型HTML页面，在弹出模式下使用混合媒体查看器。 或者，您可以在嵌入式模式下使用查看器，在该模式下，查看器会使用文档化的API集成到目标网页中。
...
混合媒体查看器支持其他移动应用程序中常见的单触和多触手势。 当查看器无法处理用户的轻扫手势时，它将事件转发到Web浏览器以执行本机页面滚动。 即使查看器占据了设备屏幕的大部分区域，用户也可以通过此功能在页面中导航。
...
<table id="table_ED747CC7178448919C34A4FCD18922D0"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>在连续缩 <span class="codeph"> 放模 </span> 式下，放大一个级别直到达到最大放大率，下一个多次点击手势将重置为初始状态。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 当当前资产是旋转集，并且图像处于重置状态时，它将水平旋转旋转旋转集。 </p> <p> 当当前资产是旋转集或图像，并且图像放大后，它将水平移动图像。 如果图像被移到视图边缘，并且该方向仍然进行轻扫，则手势将执行本机页面滚动。 </p> <p> 滚动浏览色板条中色板的列表。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>垂直轻扫或轻扫 </p> </td> 
   <td colname="col2"> <p> 如果图像处于重置状态，则在使用多维旋转集时，图像会更改垂直视图角。 在一维旋转集中，或者当多维旋转集位于最后一个或第一个轴上时，如果垂直轻扫不会导致垂直视图角度改变，则手势将执行本机页面滚动。 </p> <p> 当当前资产是旋转集或图像，并且图像放大时，垂直移动图像。 如果图像被移到视图边缘，并且该方向仍然进行轻扫，则手势将执行本机页面滚动。 </p> <p> 如果手势是在色板区域内完成的，则执行本机页面滚动。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
建议您使用现成的HTML页面进行弹出操作模式。 在这种情况下，它被调用， [!DNL MixedMediaViewer.html] 并位于标准IS查看 [!DNL html5/] 器部署的子文件夹中：
...
1. 将查看器JavaScript文件添加到网页。
   ...
   创建查看器需要在HTML头中添加脚本标记。 在使用查看器API之前，请确保包含该查看器 [!DNL MixedMediaViewer.js]。 该文 [!DNL MixedMediaViewer.js] 件位于标准IS查看 [!DNL html5/js/] 器部署的子文件夹下：
...
1. 设置查看器大小
   ...
   此查看器在处理多项目集时显示缩略图。 在桌面系统上，缩略图位于主视图下方。 同时，查看器允许在运行时使用 `setAsset()` API交换主资产。 作为开发人员，当新资产仅包含一个项目时，您可以控制查看器如何管理底部的缩略图区域。 可以保持外部查看器的大小不变，并让主视图增加其高度并占据缩略图区域。 或者，您可以保持主视图大小为静态并折叠外部查看器区域，让网页内容向上移动，然后使用缩略图中留下的免费页面空间。
   ...
   有关 [使用CSS设置查看器样式的更多信息](../../c-html5-s7-aem-asset-viewers/c-html5-mixedmedia-viewer-about/c-html5-mixedmedia-viewer-customizingviewer/c-html5-mixedmedia-viewer-customizingviewer.md#concept-61b3410f187c4bf3af09ec813c649bf4) ，请参阅自定义混合媒体查看器。
   ...
   要使主视图尺寸变为静态，请使用 `Container` CSS选择器或使用修饰符为内部 `.s7mixedmediaviewer .s7container` SDK组件定义查看器大小的绝对单 `stagesize` 位。
   ...
   您可以在Scene7 Publishing System的查看器预设记录中设置修饰符，或将修饰符与查看器初始化代码显式传递给集合，或者作为API调用进行传递，如本帮助的“命令参考”部分所述，如下所示： `stagesize``params`
...
1. 创建和初始化查看器。
   ...
   完成上述步骤后，您将创建类的一个实例，将所有配置信息传递给它的构造函数，并对查看器实例 `s7viewers.MixedMediaViewer``init()` 调用方法。 配置信息作为JSON对象传递给构造函数。 此对象至少应包含字段，该字 `containerId` 段包含查看器容器ID的名称以及查看器支持的配置参数的嵌套 `params` JSON对象。 在这种情况下，该对 `params` 象必须至少将图像服务URL作为属性传递，将视频服务器URL作为属性传递，并将初始资 `serverUrl` 产作为参数传递 `videoserverurl``asset` 。 基于JSON的初始化API允许您使用单行代码创建和开始查看器。
   ...
   以下是创建查看器实例的示例，将最小必要的配置选项传递给构造函数并调用方 `init()` 法。 示例假定 `mixedMediaViewer` 是查看器实例；是 `s7viewer` 占位符的名称 `DIV`;是 [!DNL http://s7d1.scene7.com/is/image/] 图像服务URL;是 [!DNL http://s7d1.scene7.com/is/content/] 视频服务器URL;而 [!DNL Scene7SharedAssets/Mixed_Media_Set_Sample] 且是资产：
...
以下代码是嵌入具有固定大小的混合媒体查看器的简单网页的完整示例：
...
## 无限制高度的响应式嵌入 {#section-056cb574713c4d07be6d07cf3c598839}
