---
title: 弹出缩放视图
description: 主视图由静态图像和弹出视图中显示的缩放图像组成。 它还包括在静态图像上显示的高亮导航区域，以及在静态图像顶部显示的提示消息。
...
主视图由静态图像和弹出视图中显示的缩放图像组成。 它还包括在静态图像上显示的高亮导航区域，以及在静态图像顶部显示的提示消息。
...
When `highlightmode` 参数设置为 `cursor`，则主视图中的高亮显示将替换为固定大小的光标图稿，该图稿由CSS类选择器控制：
...
>[!NOTE]
>
>光标支持 `input` 属性选择器，可用于对不同设备应用不同的光标图稿和大小。 特别是， `input="mouse"` 对应于桌面系统和 `input="touch"` 对应于触控设备。

**叠加的CSS属性**

当 `overlay` 参数设置为 `1`，则使用CSS类选择器控制高亮帧或光标图像周围的区域：
...
提示消息可以本地化。 请参阅 [用户界面元素的本地化](../../../c-html5-s7-aem-asset-viewers/c-html5-flyout-viewer-20-about/c-html5-flyout-viewer-20-localization.md#concept-6c8e58c611934e93ae3f211f46e15c27) 以了解更多信息。
