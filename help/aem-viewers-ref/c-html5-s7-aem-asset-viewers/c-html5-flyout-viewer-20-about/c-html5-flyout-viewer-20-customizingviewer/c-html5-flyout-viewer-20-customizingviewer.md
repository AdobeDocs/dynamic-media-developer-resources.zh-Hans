...
创建自定义CSS时，请记住查看器分配 `.s7flyoutviewer` 类添加到其容器DOM元素。 如果您使用的是通过 `style=` 命令，使用 `.s7flyoutviewer` 类作为CSS规则的子选择器中的父类。 如果您在网页上执行嵌入样式，还应使用容器DOM元素的ID来限定此选择器，如下所示：
...
为帮助创建响应式设计的CSS，查看器支持CSS标记，这些CSS标记是特殊的CSS类动态分配给顶级查看器容器元素。 分配基于运行时查看器大小和当前设备上使用的输入类型。
...
使用CSS标记是构建响应式设计CSS的最灵活方法。 原因在于，它不仅允许您定位设备屏幕大小，还允许您定位实际的查看器大小，这对于响应式设计的网页布局可能非常有用。
