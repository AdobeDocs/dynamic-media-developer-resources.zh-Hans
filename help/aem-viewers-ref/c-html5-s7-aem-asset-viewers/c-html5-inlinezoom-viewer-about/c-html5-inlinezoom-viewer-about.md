---
description: 内联缩放查看器是图像查看器。 当用户滚动或触碰主视图时，它显示一个静态图像，该静态图像上显示缩放版本。 此查看器可处理图像集，并可使用色板进行导航。 它设计为可在桌面和移动设备上使用。
keywords: responsive
seo-description: 内联缩放查看器是图像查看器。 当用户滚动或触碰主视图时，它显示一个静态图像，该静态图像上显示缩放版本。 此查看器可处理图像集，并可使用色板进行导航。 它设计为可在桌面和移动设备上使用。
seo-title: 内联缩放
...
内联缩放查看器是图像查看器。 当用户滚动或触碰主视图时，它显示一个静态图像，该静态图像上显示缩放版本。 此查看器可处理图像集，并可使用色板进行导航。 它设计为可在桌面和移动设备上使用。
...
内联缩放查看器表示主JavaScript文件和一组辅助文件（单个JavaScript包含该查看器在运行时下载的特定查看器、资源、CSS所使用的所有查看器SDK组件）。

内联缩放查看器既可以在弹出模式下使用随图像服务查看器提供的生产就绪型HTML页面，也可以在嵌入模式下使用，在嵌入模式下，它使用文档化的API集成到目标网页中。
...
不同网页对查看器行为的需求不同。 有时网页会提供一个可单击的链接，该链接在单独的浏览器窗口中打开查看器。 在其他情况下，可能需要将查看器直接嵌入到托管页面中。 在后一种情况下，网页可能具有静态页面布局，或者使用响应式设计，该设计在不同设备上显示不同的内容，或者针对不同的浏览器窗口大小。 为满足这些需求，查看器支持三种主要操作模式：弹出窗口、固定大小嵌入和响应式嵌入。

**弹出窗口**

在弹出模式中，查看器在单独的Web浏览器窗口或选项卡中打开。 它会占用整个浏览器窗口区域，并在调整浏览器窗口大小或更改设备方向时进行调整。

此模式是移动设备上最常见的模式。 网页使用 `window.open()` JavaScript调用、正确配置的 `A` HTML元素或任何其他合适的方式加载查看器。

建议将现成的HTML页面用于名为的弹出模式 `FlyoutViewer.html`。 它位于标准图像服 [!DNL html5/] 务查看器部署的子文件夹下：

`<s7viewers_root>/html5/FlyoutViewer.html`

还必须将FlyoutZoomView组件配置为在内联缩放模式下工作。 建议将现成预设用于内联缩放查看器， `Scene7SharedAssets/Universal_HTML5_Zoom_Inline` 或从中派生的自定义预设。 通过应用自定义CSS可以实现可视化自定义。
...
在内联缩放查看器中使用响应式设计嵌入模式时，请确保使用参数为主视图图像指定显式断 `imagereload` 点。 理想情况下，按照网页CSS的指示，将断点与查看器宽度断点相匹配。
...
1. 设置查看器大小。
   ...
   有关使 [用CSS设置查看器样式的更多信息](../../c-html5-s7-aem-asset-viewers/c-html5-inlinezoom-viewer-about/c-html5-inlinezoom-viewer-customizingviewer/c-html5-inlinezoom-viewer-customizingviewer.md#concept-82f8c71adbe54680a0c2f83f81e5f451) ，请参阅自定义内联缩放查看器。
...
1. 创建和初始化查看器。
   ...
   完成上述步骤后，您将创建类的一个实例，将所有配置信息传递给它的构造函数，并对查看器实例 `s7viewers.FlyoutViewer``init()` 调用方法。 配置信息作为JSON对象传递给构造函数。 此对象至少应包含字段，该字 `containerId` 段包含查看器容器ID的名称以及查看器支持的配置参数的嵌套 `params` JSON对象。 在这种情况下，该对 `params` 象至少必须具有作为属性传递的图像服务URL、作为参数的初始资产、将CSS作为参数加载的基本路径以及作为参数 `serverUrl` 加载的预设名 `asset``contentUrl``config` 称。 基于JSON的初始化API允许您使用单行代码创建和开始查看器。
   ...
   以下是创建查看器实例的示例，将最小必要的配置选项传递给构造函数并调用方 `init()` 法。 示例假定 `inlineZoomViewer` 是查看器实例；是 `s7viewer` 占位符的名称 `DIV`;是 `http://s7d1.scene7.com/is/image/` 图像服务URL;而 `Scene7SharedAssets/ImageSet-Views-Sample` 且是资产：
...
* 而不是使用绝对单位设置固定的查看器大小，使用CSS将查看器 `width` 设置为 `height` 100%，如下所示：
