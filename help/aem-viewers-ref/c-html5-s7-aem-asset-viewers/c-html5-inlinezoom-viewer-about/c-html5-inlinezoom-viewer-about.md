---
description: 内联缩放查看器是图像查看器。 当用户滚动或触碰主视图时，它显示静态图像，该静态图像上显示缩放版本。 此查看器可处理图像集，并可使用色板进行导航。 它设计为可在桌面和移动设备上使用。
keywords: responsive
seo-description: 内联缩放查看器是图像查看器。 当用户滚动或触碰主视图时，它显示静态图像，该静态图像上显示缩放版本。 此查看器可处理图像集，并可使用色板进行导航。 它设计为可在桌面和移动设备上使用。
seo-title: 内联缩放
...
内联缩放查看器是图像查看器。 当用户滚动或触碰主视图时，它显示静态图像，该静态图像上显示缩放版本。 此查看器可处理图像集，并可使用色板进行导航。 它设计为可在桌面和移动设备上使用。
...
内联缩放查看器表示主JavaScript文件和一组帮助文件（单个JavaScript包含该特定查看器、资源、CSS使用的所有查看器SDK组件），该查看器在运行时下载。

内联缩放查看器既可以在弹出模式中使用图像服务查看器附带的生产就绪型HTML页面，也可以在嵌入模式中使用，在嵌入模式中，使用文档化的API将其集成到目标网页中。
...
<table id="table_ED747CC7178448919C34A4FCD18922D0"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p> 激活弹出视图或在色板的主缩放级别和辅助缩放级别之间进行更改，然后选择新的缩略图。 </p> </td> 
  </tr> 
  <tr> 
...
不同的网页对查看器行为有不同的需求。 有时网页会提供可单击的链接，该链接会在单独的浏览器窗口中打开查看器。 在其他情况下，可能需要将查看器直接嵌入到托管页面中。 在后一种情况下，网页可能具有静态页面布局，或者使用响应式设计在不同设备上显示不同，或针对不同的浏览器窗口大小显示不同。 为满足这些需求，查看器支持三种主要操作模式： 弹出窗口、固定大小嵌入和响应式嵌入。

**弹出窗口**

在弹出模式下，查看器在单独的Web浏览器窗口或选项卡中打开。 它会占用整个浏览器窗口区域，并在调整浏览器窗口大小或更改设备方向时进行调整。

此模式在移动设备上最常见。 网页使用JavaScript调用、正确 `window.open()` 配置的HTML元素或任 `A` 何其他合适的方式加载查看器。

建议将现成的HTML页面用于名为的弹出模式 `FlyoutViewer.html`。 它位于标准图像 [!DNL html5/] 服务查看器部署的子文件夹下：

`<s7viewers_root>/html5/FlyoutViewer.html`

还必须将FlyoutZoomView组件配置为在内联缩放模式下工作。 建议您对内联缩放查看器使用现成 `Scene7SharedAssets/Universal_HTML5_Zoom_Inline` 预设或从中派生的自定义预设。 可通过应用自定义CSS实现可视化自定义。
...
主要用例是面向台式机或平板电脑设备的网页，以及响应式网页，这些网页会根据设备类型自动调整布局。

查看器在初始加载后不更改其大小时，将使用固定大小嵌入模式。 此选项最适合具有静态页面布局的网页。

响应式设计嵌入模式假定查看器可能需要在运行时调整大小以响应其容器的大小变化 `DIV`。 最常见的用例是将查看器添加到使用灵活页面布局的网页。

在内联缩放查看器中使用响应式设计嵌入模式时，请确保使用参数为主视图图像指定明确的断 `imagereload` 点。 理想情况下，按照网页CSS的规定将断点与查看器宽度断点相匹配。
...
1. 设置查看器大小。
   ...
   有关 [使用CSS设置查看器样式](../../c-html5-s7-aem-asset-viewers/c-html5-inlinezoom-viewer-about/c-html5-inlinezoom-viewer-customizingviewer/c-html5-inlinezoom-viewer-customizingviewer.md#concept-82f8c71adbe54680a0c2f83f81e5f451) ，请参阅自定义内联缩放查看器。
...
1. 创建和初始化查看器。
   ...
   完成上述步骤后，您将创建类的一个实例，将 `s7viewers.FlyoutViewer` 所有配置信息传递给它的构造函数，并在查看器实例 `init()` 上调用方法。 配置信息作为JSON对象传递给构造函数。 此对象至少应具有包含查 `containerId` 看器容器ID名称的字段，以及包含查看器支持 `params` 的配置参数的嵌套JSON对象。 在这种情况下，对 `params` 象必须至少将图像服务URL作为属性传 `serverUrl` 递，初始资产作为参数，将CSS作为参数加载的基 `asset` 本路径，以及预设名称作为 `contentUrl``config` 参数。 基于JSON的初始化API允许您使用单行代码创建和开始查看器。
   ...
   以下是创建查看器实例的示例，将最小必要配置选项传递给构造函数并调用方 `init()` 法。 该示例假 `inlineZoomViewer` 定是查看器实例； `s7viewer` 是占位符的名称 `DIV`; `http://s7d1.scene7.com/is/image/` 是图像服务URL; 而 `Scene7SharedAssets/ImageSet-Views-Sample` 是资产：
...
* 而不是使用绝对单位设置固定的查看器大小，而是使用CSS将 `width` 查看器 `height` 设置为100%，如下所示：
