...
通过创建自定义CSS，可实现所有可视自定义和大多数行为自定义。
...
创建自定义CSS时，请记住查看器将`.s7videoviewer`类分配给其容器DOM元素。 如果使用使用`style=`命令传递的外部CSS文件，请在CSS规则的子代选择器中使用`.s7videoviewer`类作为父类。 如果在网页上嵌入样式，则还可以通过容器DOM元素的ID来限定此选择器，如下所示：
...
以下范例CSS在具有鼠标输入的系统上将播放／暂停按钮大小设置为28 x 28像素，在触控设备上将其设置为56 x 56像素。 此外，如果查看器大小变得非常小，它会完全隐藏按钮：
...
使用CSS标记是构建响应式设计的CSS的最灵活方法，因为它不仅允许您目标设备屏幕大小，还允许实际的查看器大小，这对于响应式设计页面布局可能非常有用。
...
应用到移动查看器时，请使用四个CSS媒体查询，按照以下列顺序在CSS中定义：
...
使用CSS媒体查询方法，您应按照如下方式通过设备感知组织CSS:

* 首先，桌面特定部分定义所有属性，这些属性是桌面特定属性或所有屏幕通用属性。
* 其次，四个媒体查询应按上面定义的顺序操作，并提供特定于相应设备类型的CSS规则。
...
CSS Sprite是一种不同的方法，它将所有元素状态的图像图稿组合到一个称为“Sprite”的PNG文件中。 这样的“Sprite”具有给定元素的所有可视状态，这些元素被一个接一个地放置。 当用Sprite设置用户界面元素的样式时，CSS中的所有不同状态都引用了相同的Sprite图像。 此外，`background-position`属性用于每个状态，以指定使用“sprite”图像的哪个部分。 您可以以任何合适的方式构建“Sprite”图像。 查看器通常垂直堆叠它。 下面是一个基于“sprite”的示例，从上面设计同一个全屏按钮的样式：
...
* CSS中指向外部资源的所有路径都会根据CSS位置而非查看器HTML页面的位置进行解析。 将默认CSS复制到其他位置时，请记住考虑此规则。 复制自定义CSS中的默认资源或更新路径。
* 位图图稿的首选格式为PNG。
