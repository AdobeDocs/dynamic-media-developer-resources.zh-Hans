...
创建自定义CSS时，请记住查看器分配 `.s7videoviewer` 类到其容器DOM元素。 如果您使用随一起传递的外部CSS文件 `style=` 命令，使用 `.s7videoviewer` 类作为CSS规则的后代选择器中的父类。 如果您在网页上嵌入样式，则还可以使用容器DOM元素的ID来限定此选择器，如下所示：
...
以下示例CSS将播放/暂停按钮的大小设置为28 x 28像素（在采用鼠标输入的系统上）和56 x 56像素（在触控设备上）。 此外，如果查看器大小变得太小，则会完全隐藏按钮：
...
使用CSS标记是构建响应式设计CSS的最灵活方法。 原因在于，它不仅允许您定位设备屏幕大小，而且还可以定位实际查看器大小，这对于响应式设计页面布局可能很有用。
...
* 指向CSS内外部资源的所有路径都是针对CSS位置解析的，而不是针对查看器HTML页的位置。 将默认CSS复制到其他位置时，请记住考虑此规则。 复制默认资源或更新自定义CSS中的路径。
* 位图图稿的首选格式为PNG。
