...
清理器“旋钮”也会随着视频播放而移动，以指示视频在播放期间的当前时间位置。 视频清理器始终占据控制栏的整个宽度。 可以通过CSS构建视频洗刷的外观并更改其高度和垂直位置。
...
<table id="table_966826FB81114362A8D81D1EED38D512"> 
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 如果使用CSS sprite，则定位在图稿sprite内。 </p> <p>参见 <a href="../../../c-html5-s7-aem-asset-viewers/c-html5-video-reference/c-html5-video-viewer-20-customizingviewer/c-html5-video-viewer-20-customizingviewer.md#section-9b6d8d601cb441d08214dada7bb4eddc" format="dita" scope="local"> CSS脚本 </a>. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
可以本地化视频洗刷工具提示。 参见 [用户界面元素的本地化](../../../c-html5-s7-aem-asset-viewers/c-html5-video-reference/r-html5-video-viewer-20-localization.md#concept-1d5ca2d8480f4064a51eddba13940aad) 了解更多信息。

**示例**  — 使用具有自定义跟踪颜色的视频清理器设置视频查看器。 清理器高度必须为10像素，并且位于距控制栏上边缘和左边缘的10像素和35像素的位置。
...
使用启用视频分页时 `navigation` 参数，章节位置将作为标记显示在视频洗刷轨道的顶部。
...
>[!NOTE]
>
>此按钮同时支持 `state` 属性选择器，可用于将不同的外观应用于不同的按钮状态。 特别是， `selected='default'` 与默认视频章节标记状态和 `selected='over'` 当鼠标悬停或触摸手势激活视频章节标记时使用。

**示例**  — 设置一个像素为5 x 8的视频章节标记，并在“默认”和“超过”状态中使用不同的图片。
...
视频章节气泡位于视频章节标记的顶部，并显示给定章节的标题、开始时间和描述。 可以控制相对于视频洗刷轨道的最大气泡宽度和垂直偏移。 其余由组件自动计算。
...
**示例**  — 设置一个宽为235像素、距视频洗刷轨道底部向上八像素的视频章节气泡。
...
视频章节气泡由一个可选的标题和内容组成。 标头具有可选的章节开始时间和章节标题。
...
<table id="table_56FBC3BADDEA4E15924DD750CADC474F"> 
 <tbody> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> height </span> </p> </td> 
   <td colname="col2"> <p>视频章节泡标题高度。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> 填充 </span> </p> </td> 
   <td colname="col2"> <p>视频章节气泡标题文本的内部填充。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> background-color </span> </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>视频章节气泡标题文本行高度。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
**示例**  — 使用灰色10像素Verdana字体设置章节开始时间，且右侧有10像素填充。
...
**示例**  — 使用深灰色11像素Verdana字体及浅灰色背景设置视频章节描述。 最后，采用5像素行高、12像素水平填充、12像素顶部填充、9像素底部填充。
...
<table id="table_BC6AFB57D9404A84A3AE657448C0EB06"> 
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>楔形连接器的颜色。 </p> <p>定义为 <span class="codeph"> &lt;color&gt; 透明 </span> 以便仅定义顶部边框颜色，而其余边框保持透明。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> border-width </span> </p> </td> 
   <td colname="col2"> <p> 楔形连接器宽度。 </p> <p>定义为 <span class="codeph"> &lt;width&gt; &lt;width&gt; 0 </span> 因此，仅为顶部边框和水平边框定义相同的宽度，而底部边框宽度为 <span class="codeph"> 0 </span>. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> margin </span> </p> </td> 
   <td colname="col2"> <p> 仅定义负下边距。 它应具有与相同的值 <span class="codeph"> border-width </span>. </p> </td> 
  </tr> 
 </tbody> 
</table>
