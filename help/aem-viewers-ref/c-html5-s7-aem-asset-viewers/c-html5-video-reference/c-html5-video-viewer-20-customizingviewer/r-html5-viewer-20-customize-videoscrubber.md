---
description: 视频擦洗器是水平滑块控件，用户可通过它动态搜索当前播放的视频中的任意时间位置。
seo-description: 视频擦洗器是水平滑块控件，用户可通过它动态搜索当前播放的视频中的任意时间位置。
seo-title: 视频擦洗器
...
视频擦洗器是水平滑块控件，用户可通过它动态搜索当前播放的视频中的任意时间位置。
...
视频擦洗器工具提示可以本地化。 有关 [详细信息，请参阅用户界面元素的本地化](../../../c-html5-s7-aem-asset-viewers/c-html5-video-reference/r-html5-video-viewer-20-localization.md#concept-1d5ca2d8480f4064a51eddba13940aad) 。

**示例** -设置视频查看器，其中视频浏览条的自定轨道颜色为10像素高，距控制栏的上边缘和左边缘10像素和35像素。
...
当使用参数启用视频分章时， `navigation` 章节位置将作为标记显示在视频擦洗器轨道的顶部。
...
>[!NOTE]
>
>此按钮同时支持属 `state` 性选择器，您可以使用它将不同的外观应用到不同的按钮状态。 特别是，与默 `selected='default'` 认的视频章节标记状态相对应，并且当通过鼠标悬停或触摸手势 `selected='over'` 激活视频章节标记时，将使用该标记。

**示例** -设置5 x 8像素的视频章节标记，并将不同图稿用于“default”和“over”状态。
...
视频章节气泡位于视频章节标记的顶部，并显示给定章节的标题、开始时间和说明。 可以控制相对于视频擦洗器轨道的最大气泡宽度和垂直偏移。 其余部分由组件自动计算。
...
**示例** -设置一个视频章节气泡，该气泡的宽度为235像素，距视频擦洗器轨道的底部向上八像素。
...
**示例** -使用灰色十像素Verdana字体设置章节开始时间，右边有十像素填充。
...
**示例** -使用深灰色、11像素Verdana字体、浅灰色背景设置视频章节描述；5像素行高、12像素水平边距、12像素顶边距和9像素底边距。
...
<table id="table_BC6AFB57D9404A84A3AE657448C0EB06"> 
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>楔形连接器颜色。 </p> <p>定义为 <span class="codeph"> &lt;color&gt;透明，以 </span> 便仅定义顶部边框颜色，其余边框保持透明。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> border-width </span> </p> </td> 
   <td colname="col2"> <p> 楔形连接器宽度。 </p> <p>定义为 <span class="codeph"> &lt;width&gt; &lt;width&gt; 0，以便仅为顶部和水平边框定义相同的宽度，并且底部边框的宽度 </span> 为0 <span class="codeph"></span>。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> margin </span> </p> </td> 
   <td colname="col2"> <p> 仅定义负底边距。 它应具有与边框宽度相同 <span class="codeph"> 的值 </span>。 </p> </td> 
  </tr> 
 </tbody> 
</table>
