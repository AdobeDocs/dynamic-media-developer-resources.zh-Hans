---
description: 通过创建自定义CSS，可以为eCatalog查看器进行所有可视化自定义和大多数行为自定义。
keywords: responsive
seo-description: 通过创建自定义CSS，可以为eCatalog查看器进行所有可视化自定义和大多数行为自定义。
seo-title: 自定义eCatalog查看器
...
通过创建自定义CSS，可以为eCatalog查看器进行所有可视化自定义和大多数行为自定义。
...
创建自定义CSS时，请记住查看器将 `.s7ecatalogviewer` 类分配给其容器DOM元素。 如果使用随命令一起传递的外部CSS文 `style=` 件，请在CSS规则的 `.s7ecatalogviewer` 子代选择器中将类用作父类。 如果要在网页上执行嵌入样式，则还可以按如下方式将此选择器限定为容器DOM元素的ID:
