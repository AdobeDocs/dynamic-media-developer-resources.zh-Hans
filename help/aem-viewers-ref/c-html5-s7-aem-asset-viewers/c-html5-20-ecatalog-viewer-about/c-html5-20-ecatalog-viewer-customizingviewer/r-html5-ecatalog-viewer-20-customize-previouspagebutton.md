---
title: “上一页”按钮
description: 单击或点按此按钮会将用户转至目录中的上一页。 此按钮显示在主控制栏中。 为了节省屏幕空间，此按钮不会显示在手机上。 您可以使用CSS调整此按钮的大小、外观和位置。
...
# “上一页”按钮{#previous-page-button}

选择或点击此按钮会将用户转至目录中的上一页。 此按钮显示在主控制栏中。 为了节省屏幕空间，此按钮不会显示在手机上。 您可以使用CSS调整此按钮的大小、外观和位置。
...
示例 — 设置上一页按钮，该按钮为28 x 28像素，位于距底部4像素和距主控栏右边缘250像素的位置。 最后，为四种不同的按钮状态分别显示不同的图像。
