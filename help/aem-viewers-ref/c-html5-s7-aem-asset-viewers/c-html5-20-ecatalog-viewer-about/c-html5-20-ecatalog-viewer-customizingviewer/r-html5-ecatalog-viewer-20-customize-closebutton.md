---
title: 關閉按鈕
description: 選取或點選此按鈕會關閉內含網頁。 只有在closebutton引數設為1時，此按鈕才會出現。 此按鈕在桌上型電腦系統上無法使用。 您可以使用CSS調整此按鈕的大小、外觀和位置。
...
選取或點選此按鈕會關閉內含網頁。 只有在closebutton引數設為1時，此按鈕才會出現。 此按鈕在桌上型電腦系統上無法使用。 您可以使用CSS調整此按鈕的大小、外觀和位置。
...
範例 — 設定56 x 56畫素的「關閉」按鈕，且此按鈕位於主控制列的頂端和右邊緣4畫素處。 最後，針對四種不同按鈕狀態分別顯示不同的影像。
