---
description: 单击或点按此按钮可将用户转到目录中的上一页。 此按钮显示在主控制栏中。 此按钮不显示在手机上以节省屏幕资产。 您可以使用CSS调整按钮的大小、外观和位置。
...
# “大上一页”按钮{#large-previous-page-button}

单击或点按此按钮可将用户转到目录中的上一页。 此按钮显示在主控制栏中。 此按钮不显示在手机上以节省屏幕资产。 您可以使用CSS调整按钮的大小、外观和位置。
...
示例 — 设置一个大的上一页按钮，其大小为56 x 56像素，垂直居中并定位到左查看器边框，并针对四个不同按钮状态中的每个状态显示一个不同的图像。
