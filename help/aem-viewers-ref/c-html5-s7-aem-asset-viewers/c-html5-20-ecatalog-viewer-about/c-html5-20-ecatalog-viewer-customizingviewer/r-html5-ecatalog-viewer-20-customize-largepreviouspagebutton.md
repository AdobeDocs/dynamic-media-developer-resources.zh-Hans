---
title: 上一頁大按鈕
description: 按一下或點選此按鈕會將使用者導向到目錄中的上一頁。 此按鈕會出現在主控制列中。 此按鈕不會顯示在行動電話上，以節省熒幕空間。 您可以使用CSS調整此按鈕的大小、外觀和位置。
...
# 上一頁大按鈕{#large-previous-page-button}

選取或點選此按鈕會將使用者導向到目錄中的上一頁。 此按鈕會出現在主控制列中。 此按鈕不會顯示在行動電話上，以節省熒幕空間。 您可以使用CSS調整此按鈕的大小、外觀和位置。
...
範例 — 設定一個56 x 56畫素的大型上一頁按鈕，垂直置中並錨定至左側檢視器邊框，並為四種不同按鈕狀態分別顯示不同的影像。
