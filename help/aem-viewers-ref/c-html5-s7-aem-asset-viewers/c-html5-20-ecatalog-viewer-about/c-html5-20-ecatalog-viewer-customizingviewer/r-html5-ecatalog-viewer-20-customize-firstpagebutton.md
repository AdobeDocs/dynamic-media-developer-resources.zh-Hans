...
选择或点按此按钮可将用户转到目录中的第一页。 此按钮显示在台式机系统和平板电脑的主控制栏中；在手机上，它会添加到辅助控制栏。 您可以使用CSS调整按钮的大小、外观和位置。
...
示例 — 设置第一个页面按钮，其长度为28 x 28像素，距底部4像素，距主控制栏左边缘220像素。 最后，针对四个不同的按钮状态中的每个状态显示一个不同的图像。
