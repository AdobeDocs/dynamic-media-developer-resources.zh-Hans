---
title: 縮小顯示按鈕
description: 按一下或點選此按鈕可縮小主檢視中的影像。 此按鈕不會顯示在行動電話上，以節省熒幕空間。 您可以使用CSS調整此按鈕的大小、外觀和位置。
...
選取或點選此按鈕可縮小主檢視中的影像。 此按鈕不會顯示在行動電話上，以節省熒幕空間。 您可以使用CSS調整此按鈕的大小、外觀和位置。
...
範例 — 設定縮小顯示按鈕，其位置為28 x 28畫素，距主控列的底部4畫素，距右邊緣75畫素。 最後，針對四種不同按鈕狀態分別顯示不同的影像。
