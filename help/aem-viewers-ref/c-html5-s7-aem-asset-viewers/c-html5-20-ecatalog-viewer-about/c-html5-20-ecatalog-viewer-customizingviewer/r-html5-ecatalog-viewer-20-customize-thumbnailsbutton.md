---
title: 「縮圖」按鈕
description: 按一下或點選此按鈕會重設檢視器，在主檢視和縮圖之間切換。 此按鈕會出現在主控制列中。 您可以使用CSS調整此按鈕的大小、外觀和位置。
...
選取或點選此按鈕會重設檢視器在主要檢視和縮圖之間切換。 此按鈕會出現在主控制列中。 您可以使用CSS調整此按鈕的大小、外觀和位置。
...
範例 — 若要設定縮圖按鈕，其位置為28 x 28畫素，且距離底部僅4畫素，距離主控列的左邊緣僅5畫素。 最後，選取或未選取時，針對四種不同按鈕狀態顯示不同的影像。
