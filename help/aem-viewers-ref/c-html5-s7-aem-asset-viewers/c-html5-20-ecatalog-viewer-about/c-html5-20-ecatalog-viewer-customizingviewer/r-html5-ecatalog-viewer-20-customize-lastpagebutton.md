---
description: 单击或点按此按钮可将用户转到目录中的最后一页。 此按钮显示在台式机系统和平板电脑的主控制栏中；在手机上，它会添加到辅助控制栏。 您可以使用CSS调整按钮的大小、外观和位置。
...
单击或点按此按钮可将用户转到目录中的最后一页。 此按钮显示在台式机系统和平板电脑的主控制栏中；在手机上，它会添加到辅助控制栏。 您可以使用CSS调整按钮的大小、外观和位置。
...
示例 — 设置一个最后一页按钮，其大小为28 x 28像素，位于底部4像素和主控制栏左边缘220像素，并针对四个不同按钮状态中的每一个状态显示一个不同的图像。
