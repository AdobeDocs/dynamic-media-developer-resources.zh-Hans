---
title: “最后一页”按钮
description: 单击或点按此按钮会将用户转到目录中的最后一页。 此按钮显示在桌面系统和平板电脑的主控制栏中；在手机上，此按钮将添加到辅助控制栏中。 您可以使用CSS调整此按钮的大小、外观和位置。
...
选择或点击此按钮会将用户转至目录中的最后一页。 此按钮显示在桌面系统和平板电脑的主控制栏中；在手机上，此按钮将添加到辅助控制栏中。 您可以使用CSS调整此按钮的大小、外观和位置。
...
示例 — 设置一个28 x 28像素的最后页按钮，该按钮位于距底部4像素和距主控制栏左边缘220像素的位置。 最后，为四种不同的按钮状态分别显示不同的图像。
