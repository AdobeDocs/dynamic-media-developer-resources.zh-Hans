...
示例 — 要设置带有内边距的对话框标题，请添加24 x 14像素图标和粗体16点标题。 以及一个28 x 28像素的“关闭”按钮，该按钮位于距离顶部两个像素的位置，距离对话框容器右侧两个像素的位置：
...
示例 — 要将下拉按钮设置为28 x 28像素，并为每种状态使用单独的图像：
...
示例 — 要设置一个宽度为28像素的滚动条，并且该滚动条在滚动面板的顶部、右侧和底部各有8个像素的边距：
...
示例 — 要设置一个滚动条缩略图，其大小为28 x 45像素，顶部和底部各有10个像素边距，并且每种状态都有不同的图稿：
...
示例 — 要设置28 x 32像素的滚动按钮，并为每种状态设置不同的图稿：
