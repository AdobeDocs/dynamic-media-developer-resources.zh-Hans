---
description: 单击或点按此按钮会重置主视图中的图像。 此按钮显示在台式机系统和平板电脑的主控制栏中。 在移动电话上，此按钮显示在图像的底部中心。 但是，当图像处于重置状态时，不会显示该图像。 您可以使用CSS调整按钮的大小、外观和位置。
...
单击或点按此按钮会重置主视图中的图像。 此按钮显示在台式机系统和平板电脑的主控制栏中。 在移动电话上，此按钮显示在图像的底部中心。 但是，当图像处于重置状态时，不会显示该图像。 您可以使用CSS调整按钮的大小、外观和位置。
...
示例 — 要设置一个28 x 28像素的缩放重置按钮，该按钮位于（在桌面上）距主控制栏底部4像素和右边缘47像素，并针对四个不同按钮状态中的每个状态显示一个不同的图像。
