---
title: 缩放重置按钮
description: 选择或点按此按钮会重置主视图中的图像。 此按钮显示在台式机系统和平板电脑的主控制栏中。 在移动电话上，此按钮显示在图像的底部中心。 但是，当图像处于重置状态时，不会显示该图像。 您可以使用CSS调整按钮的大小、外观和位置。
...
选择或点按此按钮会重置主视图中的图像。 此按钮显示在台式机系统和平板电脑的主控制栏中。 在移动电话上，此按钮显示在图像的底部中心。 但是，当图像处于重置状态时，不会显示该图像。 您可以使用CSS调整按钮的大小、外观和位置。
...
示例 — 设置一个缩放重置按钮，该按钮为28 x 28像素，位于（在桌面上）从底部4像素和从主控制栏右边缘47像素的位置。 最后，针对四个不同的按钮状态中的每个状态显示一个不同的图像。
