---
title: eCatalog
description: eCatalog Viewer是一种目录查看器，以跨页或逐页方式在跨页中显示电子小册子。 eCatalog允许用户使用其他用户界面元素或专用缩略图模式浏览目录。 用户还可以放大每个页面以了解更多详细信息。
keywords: 响应式
...
eCatalog Viewer是一种目录查看器，以跨页或逐页方式在跨页中显示电子小册子。 eCatalog允许用户使用其他用户界面元素或专用缩略图模式浏览目录。 用户还可以放大每个页面以了解更多详细信息。
...
eCatalog查看器表示一个主JavaScript文件和一组帮助程序文件（单个JavaScript包含在此特定查看器、资产、CSS使用的所有Viewer SDK组件中），这些文件由查看器在运行时下载

您可以使用随IS-Viewers提供的生产就绪型HTML页在弹出模式下使用eCatalog查看器，也可以使用嵌入式模式使用，在嵌入式模式下，使用文档记录的API将其集成到Target网页中。
...
eCatalog查看器支持其他移动设备应用程序中常见的以下触控手势。
...
此查看器可完全通过键盘访问，如中所述 [键盘辅助功能和导航](../../c-keyboard-accessibility.md#topic-f5650e9493404e55a3627c8d1366b861).

## 使用eCatalog查看器的社交媒体共享工具 {#section-eb575084a99647c3a9591f439f40b412}

eCatalog查看器支持社交共享工具。 它们可用作主控制栏中的按钮，当用户单击或点按时，该按钮会扩展为共享工具栏。
...
建议您将现成的HTML页用于弹出窗口操作模式。 在这种情况下，将调用 [!DNL eCatalogViewer.html] 且位于 [!DNL html5/] 标准IS-Viewers部署的子文件夹：
...
在响应式设计嵌入模式下，查看器的行为方式有所不同，具体取决于网页确定其容器大小的方式 `DIV`. 如果网页仅设置容器的宽度 `DIV`如果不限制高度，查看器会根据所用资源的纵横比自动选择高度。 此功能可确保资源完全适合视图，而不用填充边距。 此用例最常用于使用响应式布局框架(如Bootstrap和Foundation)的网页。
...
1. 将查看器JavaScript文件添加到网页。
   ...
   创建查看器需要您在HTML头中添加脚本标记。 在使用查看器API之前，请确保包括 [!DNL eCatalogViewer.js]. 此 [!DNL eCatalogViewer.js] 文件位于 [!DNL html5/js/] 标准IS-Viewers部署的子文件夹：
...
1. 设置查看器大小
   ...
   通过声明查看器的静态大小，可以将其设置为 `.s7ecatalogviewer` 以绝对单位表示的顶级CSS类，或者使用 `stagesize` 修饰符。

   您可以直接在“HTML”页面上设置CSS大小。 或者，将大小调整放入自定义查看器CSS文件中，该文件随后将分配给Dynamic Media Classic中的查看器预设记录，或者使用样式命令显式传递。
...
1. 正在初始化查看器。
   ...
   完成上述步骤后，您将创建一个实例 `s7viewers.eCatalogViewer` 类，将所有配置信息传递到其构造函数，并调用 `init()` 方法。 配置信息作为JSON对象传递给构造函数。 至少，此对象具有 `containerId` 保存查看器容器ID名称并嵌套的字段 `params` 具有查看器支持的配置参数的JSON对象。 在本例中， `params` 对象必须至少将图像服务URL传递为 `serverUrl` 资产，而初始资产为 `asset` 参数。 基于JSON的初始化API允许您使用一行代码创建和启动查看器。
   ...
   以下示例介绍了如何创建查看器实例、将所需的最少配置选项传递给构造函数，以及调用 `init()` 方法。 此示例假定 `eCatalogViewer` 是查看器实例； `s7viewer` 是占位符的名称 `DIV`； `https://s7d1.scene7.com/is/image/` 是图像服务URL，并且 `Viewers/Pluralist` 是资产：
   ...
   以下代码是嵌入eCatalog查看器且大小固定的普通网页的完整示例：
...
在定义宽度和高度的灵活大小嵌入中，网页的样式不同。 即，它为“托架”提供两种尺寸 `DIV` 并将其居中在浏览器窗口中。 此外，该网页还设置 `HTML` 和 `BODY` 元素至100%：
