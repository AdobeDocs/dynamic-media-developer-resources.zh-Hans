...
查看器中每个可以本地化的文本内容都由一个名为SYMBOL的特殊查看器SDK标识符表示。 任何SYMBOL都具有英语区域设置( `"en"`)，并且可能还根据需要为所需数量的区域设置自定义值。
...
网页代码应将此类本地化对象作为值传递给查看器构造函数 `localizedTexts` 配置对象的字段。 另一种选择是通过调用 `setLocalizedTexts(localizationInfo)` 方法。
...
<table id="table_58C40353B7244335872350C98DF2CFB3"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>查看器在加载时生成的页面标签。 </p> <p>该符号的名称是模板，其中 <span class="codeph"> XX </span> 是横向从零开始的跨页索引，并且是可选的 <span class="codeph"> YY </span> 是目标分布内的一个从零开始的页面索引， <span class="codeph"> XX </span>. </p> <p>仅适用于初始加载的资产；如果使用 <span class="codeph"> setAsset() </span> API调用。 </p> </td> 
  </tr> 
  <tr> 
