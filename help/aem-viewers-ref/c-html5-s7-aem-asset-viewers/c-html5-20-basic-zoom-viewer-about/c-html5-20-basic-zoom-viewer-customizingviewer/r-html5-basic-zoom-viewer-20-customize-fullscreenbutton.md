---
title: 全熒幕按鈕
description: 讓檢視器在使用者按一下時進入或退出全熒幕模式。 如果檢視器在快顯視窗模式下運作且系統不支援原生全熒幕，則不會顯示此按鈕。 您可以使用CSS調整此按鈕的大小、外觀和位置。
...
範例 — 設定32 x 32畫素的全熒幕按鈕，並從檢視器的上邊緣和右邊緣放置6個畫素。 選取或未選取時，必須針對四種不同按鈕狀態顯示不同的影像：
