---
description: 基本缩放查看器的所有可视化自定义和大多数行为自定义都是通过创建自定义CSS完成的。
keywords: responsive
seo-description: 基本缩放查看器的所有可视化自定义和大多数行为自定义都是通过创建自定义CSS完成的。
...
基本缩放查看器的所有可视化自定义和大多数行为自定义都是通过创建自定义CSS完成的。
...
创建自定义CSS时，请记住查看器将 `.s7basiczoomviewer` 类分配给其容器DOM元素。 如果使用与命令一起传递的外部CSS `style=` 文件，请在 `.s7basiczoomviewer` CSS规则的子代选择器中使用类作为父类。 如果要在网页上执行嵌入样式，则还可以按如下方式将此选择器限定为容器DOM元素的ID:
...
第二组CSS标记包含 `.s7mouseinput` 和 `.s7touchinput`。 `.s7touchinput` is set if current device has touch input capabilities;否则， `.s7mouseinput` 将使用。 这些标记主要用于为不同的输入类型创建具有不同屏幕尺寸的用户界面输入元素，因为触摸输入通常需要较大的元素。 如果设备同时具有鼠标输入和触摸功能，则设 `.s7touchinput` 置该设备，并且查看器呈现适合触控的用户界面。

以下范例CSS将鼠标输入系统上的按钮大小设置为28 x 28像素，将触控设备上的按钮大小设置为56 x 56像素。 此外，如果查看器的大小很小，它会完全隐藏按钮：
...
使用CSS标记是构建响应式设计的CSS最灵活的方式，因为它不仅允许您目标设备屏幕大小，而且可以实际的查看器大小，这对于响应式设计的布局非常有用。
