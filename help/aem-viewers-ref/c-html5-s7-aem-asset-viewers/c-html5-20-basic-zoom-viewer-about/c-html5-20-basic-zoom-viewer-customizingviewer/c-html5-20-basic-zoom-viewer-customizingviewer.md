...
创建自定义CSS时，请牢记查看器将`.s7basiczoomviewer`类分配给其容器DOM元素。 如果使用通过`style=`命令传递的外部CSS文件，请在CSS规则的子代选择器中使用`.s7basiczoomviewer`类作为父类。 如果要在网页上执行嵌入样式，则还可以按如下方式将此选择器与容器DOM元素的ID一起限定：
...
第二组CSS标记包含`.s7mouseinput`和`.s7touchinput`。 `.s7touchinput` is set if current device has touch input capabilities;否则， `.s7mouseinput` 将使用。这些标记主要用于为不同的输入类型创建屏幕大小不同的用户界面输入元素，因为触摸输入通常需要较大的元素。 如果设备同时具备鼠标输入和触摸功能，则设置`.s7touchinput`，查看器将呈现适合触摸的用户界面。
...
使用CSS标记是构建响应式设计的CSS最灵活的方法，因为它不仅允许您目标设备屏幕大小，还可以实际的查看器大小，这对于响应式设计的布局非常有用。
