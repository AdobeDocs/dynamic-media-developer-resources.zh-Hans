---
title: 缩放重置按钮
description: 选择此按钮会重置主视图中的图像。 您可以使用CSS调整按钮的大小、外观和位置。
...
选择此按钮会重置主视图中的图像。 您可以使用CSS调整按钮的大小、外观和位置。
...
示例 — 设置一个32 x 32像素的缩放重置按钮，该按钮距查看器的上边缘和右边缘有6个像素。 它必须针对四个不同的按钮状态中的每一个状态显示一个不同的图像。
