---
title: 缩放重置按钮
description: 选择此按钮可重置主视图中的图像。 您可以使用CSS调整此按钮的大小、外观和位置。
...
选择此按钮可重置主视图中的图像。 您可以使用CSS调整此按钮的大小、外观和位置。
...
示例 — 设置一个32 x 32像素的缩放重置按钮，该按钮位于距离查看器上边缘和右边缘六个像素的位置。 它必须为四种不同的按钮状态中的每一种显示不同的图像。
