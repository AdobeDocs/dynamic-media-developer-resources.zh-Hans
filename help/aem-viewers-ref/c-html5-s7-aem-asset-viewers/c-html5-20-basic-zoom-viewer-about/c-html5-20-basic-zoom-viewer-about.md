...
基本缩放查看器表示查看器在运行时下载的一个JavaScript文件和一组帮助程序文件。 本质上，它是一个包含的JavaScript，其中包含此特定查看器、资源和CSS使用的所有Viewer SDK组件。
...
建议您为弹出操作模式使用现成的HTML页面。 在这种情况下，该文件夹名为[!DNL BasicZoomViewer.html]，位于标准IS-Viewers部署的[!DNL html5/]子文件夹中：
...
1. 正在将查看器JavaScript文件添加到您的网页。
   ...
   创建查看器需要您在HTML头中添加脚本标记。 在使用查看器API之前，请确保包括[!DNL BasicZoomViewer.js]。 [!DNL BasicZoomViewer.js]文件位于标准IS-Viewers部署的[!DNL html5/js/]子文件夹下：
