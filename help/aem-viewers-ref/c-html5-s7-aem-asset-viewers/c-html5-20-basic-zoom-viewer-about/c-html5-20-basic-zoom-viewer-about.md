...
基本缩放查看器表示查看器运行时下载的一个JavaScript文件和一组帮助程序文件。 本质上，它是一个JavaScript包，其中包含此特定查看器、资源和CSS使用的所有Viewer SDK组件。

您可以在弹出模式下使用随IS-Viewers提供的生产就绪型HTML页，或者在嵌入模式下使用基本缩放查看器，其中使用文档记录的API将其集成到目标网页中。
...
弹出模式是移动设备中最常见的一种模式。 网页使用加载查看器 `window.open()` JavaScript调用，正确配置 `A` HTML元素或任何其他适当的方法。

建议您为弹出操作模式使用现成的HTML页面。 在这种情况下，将其命名为 [!DNL BasicZoomViewer.html] 且位于 [!DNL html5/] 标准IS-Viewers部署的子文件夹：
...
1. 将查看器JavaScript文件添加到网页。
   ...
   创建查看器需要您在HTML头中添加脚本标记。 在使用查看器API之前，请确保包含 [!DNL BasicZoomViewer.js]. 此 [!DNL BasicZoomViewer.js] 文件位于 [!DNL html5/js/] 标准IS-Viewers部署的子文件夹：
...
1. 设置查看器大小
   ...
   您可以通过声明查看器的静态大小来为其设置静态大小。 `.s7basiczoomviewer` 顶级CSS类（以绝对单位表示），或者使用 `stagesize` 修饰符。
   ...
   您可以设置 `stagesize` 在Dynamic Media Classic中的查看器预设记录中的修饰符。 或者，您可以使用查看器初始化代码显式传递它 `params` 收藏集，或者，如“命令引用”一节中所述，作为API调用，如下所示：
...
1. 创建和初始化查看器。
   ...
   完成上述步骤后，您将创建一个实例 `s7viewers.BasicZoomViewer` 类，将所有配置信息传递到其构造函数，并调用 `init()` 方法。 配置信息作为JSON对象传递给构造函数。 此对象至少应具有containerId字段，该字段包含查看者的名称 `container ID` 和嵌套 `params` 包含查看器支持的配置参数的JSON对象。 在本例中， `params` 对象必须至少将图像服务URL传递为 `serverUrl` 资产，而初始资产为 `asset` 参数。 基于JSON的初始化API允许您通过一行代码创建和启动查看器。

   必须将查看器容器添加到DOM，以便查看器代码可以按其ID查找容器元素。 某些浏览器会延迟构建DOM，直到网页结尾。 要获得最大的兼容性，请调用 `init()` 紧靠结束位置之前的方法 `BODY` 标记上，或主体上 `onload()` 事件。

   同时，容器元素还不一定是网页布局的一部分。 例如，可以使用隐藏该内容 `display:none` 为其分配的样式。 在这种情况下，查看器会延迟其初始化过程，直到网页将容器元素带回布局为止。 发生此事件时，查看器加载会自动恢复。

   以下示例用于创建查看器实例，将最低必要的配置选项传递给构造函数并调用 `init()` 方法。 此示例假定 `basicZoomViewer` 是查看器实例； `s7viewer` 是占位符的名称 `DIV`； `http://s7d1.scene7.com/is/image/` 是图像服务URL，并且 `Scene7SharedAssets/Backpack_B` 是资产：
