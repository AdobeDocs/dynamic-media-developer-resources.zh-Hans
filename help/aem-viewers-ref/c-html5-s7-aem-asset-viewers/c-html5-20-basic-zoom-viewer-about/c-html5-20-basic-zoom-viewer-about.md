...
您可以使用随IS-Viewer提供的生产就绪型HTML页面，在弹出模式下使用基本缩放查看器，或者在嵌入模式下使用文档化API将其集成到目标网页中。
...
基本缩放查看器支持以下在其他移动应用程序中常见的触控手势。

当查看器无法处理用户的轻扫手势时，它将事件转发到Web浏览器以执行本机页面滚动。 即使查看器占据设备的大部分屏幕区域，这种功能也允许用户在页面中导航。
...
<table id="table_ED747CC7178448919C34A4FCD18922D0"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 如果图像处于重置状态，则手势将执行本机页面滚动。 </p> <p> 放大图像后，图像会移动。 如果将图像移动到视图边缘，并在该方向上执行轻扫，则手势将执行本机页面滚动。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
建议您使用现成的HTML页面进行弹出操作模式。 在这种情况下，它称为[!DNL BasicZoomViewer.html]，位于标准IS-Viewers部署的[!DNL html5/]子文件夹中：
...
1. 将查看器JavaScript文件添加到网页。
   ...
   创建查看器需要在HTML头中添加脚本标签。 在使用查看器API之前，请确保包含[!DNL BasicZoomViewer.js]。 [!DNL BasicZoomViewer.js]文件位于标准IS-Viewers部署的[!DNL html5/js/]子文件夹下：
...
1. 设置查看器大小
   ...
   您可以通过以下两种方式设置查看器的静态大小：以绝对单位为`.s7basiczoomviewer`顶级CSS类声明查看器，或使用`stagesize`修饰符。
...
1. 创建和初始化查看器。
   ...
   完成上述步骤后，您将创建`s7viewers.BasicZoomViewer`类的实例，将所有配置信息传递给它的构造函数，并对查看器实例调用`init()`方法。 配置信息作为JSON对象传递给构造函数。 此对象至少应包含containerId字段，该字段包含查看器`container ID`的名称以及查看器支持的配置参数嵌套的`params` JSON对象。 在这种情况下，`params`对象必须至少具有作为`serverUrl`属性传递的图像服务URL，以及作为`asset`参数传递的初始资产。 基于JSON的初始化API允许您使用单行代码创建和开始查看器。
   ...
   以下是创建查看器实例的示例，将最小必要配置选项传递给构造函数并调用`init()`方法。 该示例假定`basicZoomViewer`为查看器实例；`s7viewer`是占位符`DIV`的名称；`http://s7d1.scene7.com/is/image/`是图像服务URL，`Scene7SharedAssets/Backpack_B`是资产：
   ...
   以下代码是嵌入具有固定大小的基本缩放查看器的次要网页的完整示例：
