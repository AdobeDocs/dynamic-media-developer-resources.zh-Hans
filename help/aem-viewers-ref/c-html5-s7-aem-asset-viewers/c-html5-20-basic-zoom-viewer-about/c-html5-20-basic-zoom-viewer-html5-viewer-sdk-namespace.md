...
查看器用于加载和初始化SDK组件的命名空间取决于查看器操作的环境。 如果查看器在AEM(Adobe Experience Manager)中运行，则查看器将SDK组件加载到 `s7viewers.s7sdk` 命名空间中。 从SPS提供的查看器将SDK加载到中 `s7classic.s7sdk`。
...
例如，如果您计划侦听事件，并且查看器由AEM提供，则完全限定的事件类型为 `StatusEvent.NOTF_VIEW_READY``s7viewers.s7sdk.event.StatusEvent.NOTF_VIEW_READY`，并且事件监听器代码类似于以下内容：
