...
查看器用于加载和初始化SDK组件的命名空间取决于查看器运行的环境。 如果查看器在Adobe Experience Manager中运行，则查看器会将SDK组件加载到 `s7viewers.s7sdk` 命名空间。 Dynamic Media Classic提供的查看器会将SDK加载到 `s7classic.s7sdk`.
...
例如，如果您计划侦听 `StatusEvent.NOTF_VIEW_READY` 事件且查看器由Experience Manager提供服务，完全限定的事件类型为 `s7viewers.s7sdk.event.StatusEvent.NOTF_VIEW_READY`，则事件侦听器代码类似于以下内容：
