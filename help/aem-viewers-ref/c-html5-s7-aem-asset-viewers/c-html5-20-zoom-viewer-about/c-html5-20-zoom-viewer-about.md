...
<table id="table_ED747CC7178448919C34A4FCD18922D0"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 滚动浏览色板条中色板的列表。 </p> <p> 如果图像处于重置状态，且帧过渡 <span class="codeph"> 参 </span> 数设置为滑动，则资产会随幻灯片动画一起更改。 对于其 <span class="codeph"> 他帧 </span> 转换模式，手势执行本机页面滚动。 </p> <p> 如果图像被放大，则图像会水平移动图像。 如果图像移动到视图边缘，并且按相同方向执行轻扫，则手势将执行本机页面滚动。 </p> </td> 
  </tr> 
  <tr> 
...
建议您使用现成的HTML页面进行弹出操作模式。 现成的HTML页被调用，它 `ZoomViewer.html` 位于标准IS `html5/` 查看器部署的子文件夹下，如下所示：
...
响应式设计嵌入模式假定由于查看器容器的大小发生变化，在运行时必须调整查看器的大小 `DIV`。 最常见的用例是将查看器添加到使用灵活布局的网页。

在响应式设计嵌入模式中，查看器的行为方式取决于网页调整其容器的方式 `DIV`。 如果网页仅设置容器的宽度，而 `DIV`保持高度不受限制，则查看器会根据所使用的资产的长宽比自动选择其高度。 此逻辑确保资产完美地适合视图，两侧不加边距。 对于使用响应式布局框架(如Bootstrap、基础等)的网页，此用例最为常见。

如果网页同时设置查看器容器的宽度和高度，则查 `DIV`看器将填充该区域，并遵循网页提供的大小。 例如，将查看器嵌入到模态叠加中，其中叠加根据Web浏览器窗口大小进行大小调整。
...
1. 将查看器JavaScript文件添加到网页。
   ...
   创建查看器需要在HTML头中添加脚本标记。 在使用查看器API之前，请确保包含该查看器 [!DNL ZoomViewer.js]。 文 [!DNL ZoomViewer.js] 件位于标准IS [!DNL html5/js/] 查看器部署的子文件夹下：
...
1. 设置查看器大小。
   ...
   此查看器在处理多项目集时显示缩略图，在桌面系统上，缩略图放在主视图下。 同时，查看器允许在运行时使用API交换主资 `setAsset()` 产。 作为开发人员，您可以控制当新资产只有一个项目时查看器管理底部缩略图区域的方式。 可以保持外部查看器的大小不变，并让主视图增加其高度并占用缩览图区域。 或者，您可以保持主视图大小为静态并折叠外部查看器区域，让网页内容向上移动，并使用缩略图中剩余的免费屏幕空间。

   要保持外部查看器边界不变，请以绝 `.s7zoomviewer` 对单位定义顶级CSS类的大小。 CSS中的大小调整可以直接放在HTML页面上，也可以放在自定义查看器CSS文件中，该文件稍后会分配到Scene7 Publishing System中的查看器预设记录，或使用样式命令显式传递。

   有关使 [用CSS设置查看器样式](../../c-html5-s7-aem-asset-viewers/c-html5-20-zoom-viewer-about/c-html5-20-zoom-viewer-customizingviewer/c-html5-20-zoom-viewer-customizingviewer.md#concept-73a8546acdb444a387c49969ceca57d0) ，请参阅自定义缩放查看器。
   ...
   要使主视图维保持静态，请使用CSS选择器或使用修饰符，以内部SDK组 `Container` 件的绝对单 `.s7zoomviewer` 位定义 `.s7container``stagesize` 查看器大小。
   ...
   您可以在Scene7 Publishing `stagesize``params` System的查看器预设记录中设置修饰符，也可以将修饰符与查看器初始化代码显式地与集合一起传递，或作为API调用进行传递，如本帮助的“命令参考”部分中所述，如下所示：
...
1. 创建和初始化查看器。
   ...
   完成上述步骤后，您将创建类的一个实例，将 `s7viewers.ZoomViewer` 所有配置信息传递给它的构造函数，并在查看器实例 `init()` 上调用方法。

   配置信息作为JSON对象传递给构造函数。 此对象至少应包含 `containerId` 包含查看器容器ID名称的字段，以及包含查看器支持 `params` 的配置参数的嵌套JSON对象。 在这种情况下，对 `params` 象必须至少将图像服务URL作为属性传 `serverUrl` 递，并将初始资产作为 `asset` 参数。 基于JSON的初始化API允许您使用一行代码创建和开始查看器。
   ...
   以下是创建查看器实例、将最小必要配置选项传递给构造函数并调用方法的示 `init()` 例。 此示例假 `zoomViewer` 定是查看器实例、 `s7viewer` 占位符DIV的名称、 `http://s7d1.scene7.com/is/image/` 图像服务URL以及 `Scene7SharedAssets/ImageSet-Views-Sample` 资产。
