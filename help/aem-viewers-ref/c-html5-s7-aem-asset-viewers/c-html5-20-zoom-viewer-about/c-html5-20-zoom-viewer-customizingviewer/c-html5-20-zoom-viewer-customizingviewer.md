---
title: 自定义缩放查看器
description: 缩放查看器的所有可视化自定义项和大多数行为自定义项均通过创建自定义CSS来完成。
keywords: 响应式
...
缩放查看器的所有可视化自定义项和大多数行为自定义项均通过创建自定义CSS来完成。
...
创建自定义CSS时，请记住，查看器分配 `.s7zoomviewer` 类到其容器DOM元素。 如果您使用传递的外部CSS文件 `style=` 命令，使用 `.s7zoomviewer` 类作为CSS规则的后代选择器中的父类。 如果您在网页上执行嵌入样式，则还可以使用容器DOM元素的ID来限定此选择器，如下所示：
...
为了帮助创建响应式设计的CSS，查看器支持CSS标记。 这些标记是特殊的CSS类。 这些类会根据运行时查看器大小和当前设备上使用的输入类型，动态地分配给顶级查看器容器元素。
...
以下示例CSS将放大按钮大小设置为28 x 28像素（在具有鼠标输入的系统上）和56 x 56像素（在触控设备上）。 此外，如果查看器太小，它将完全隐藏按钮：
...
使用经典样式设置方法时，对于用户界面元素的每个状态，CSS将对服务器上的单个图像文件进行单独引用。 以下是样式放大按钮的CSS示例：
