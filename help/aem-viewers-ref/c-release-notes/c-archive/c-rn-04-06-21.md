...
* 增加了对服务器端DPR优化的支持。
* 添加了对清理URL配置参数的支持。 弹出查看器现在忽略查询字符串中定义的URL配置参数。 如果参数值中的域与基本查看器域不匹配或不属于允许的域列表，则会完成此功能。
* 已将产品品牌更新为Adobe Experience Viewers。
* 修复了在CSS中禁用某个子组件时，SocialShare下拉列表中的键盘导航问题。
* 修复了在iOS9和更早设备上使用海报时出现JavaScript错误的问题。
* 修复了将当前视频播放时间报告为NaN的错误。<!--  (CQ-4310148) -->
...
* 首次访客时，海报图像不会显示在Microsoft® Edge浏览器中。
