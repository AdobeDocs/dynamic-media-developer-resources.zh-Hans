...
**重要信息：** 默认情况下，最新版本的Firefox会阻止AdobeFlash插件在网站上运行。 如果您已在桌面系统上部署使用AdobeFlash插件的Scene7视频播放器，则可以使用以下三个选项：

* 用户在Firefox中查看的网页顶部会看到提示。 在提示中，用户可以选择继续阻止插件，或允许插件运行，在这种情况下，视频会照常播放。
...
* [eCatalog] 在Internet Explorer 9上将JavaScript模板用于外部链接后，图像映射卡住。
...
* CSS样式 `display:none` on `DIV` 不支持容器。 包括Jquery `hide()` 方法。
