...
* 通过将旧版移动设备HTML5查看器的SDK版本更新为3.10，修复了XSS问题。Dynamic Media使用的新开箱即用的HTML5查看器不受此XSS问题影响，因此不会更新。
...
* 添加了对被动事件侦听器的支持。
* 更新了DimensionalViewer中的3D渲染库，以更好地支持模型彩色化。
* 已禁用非活动按钮的聚焦。
* 改进了共享工具中的键盘控制和辅助技术支持。
* 更正了iOS设备上的样本滚动行为。
* 更正了eCatalog查看器中对iOS设备产生的翻页效果。
* 更正了在触控设备上启用样本的查看器中的`swatchoverlay`修饰符行为。
* 更正了MixedMediaViewer中对视频字幕的支持，以防止视频在媒体集中排在第一位。
