...
* 为Adobe Marketing Cloud集成添加了Adobe Analytics组织ID支持。
* 已将AppMeasurement JavaScript库更新到版本1.6.1。
* 现在，搜索功能已添加到eCatalog查看器中。
* 添加了HLS流视频播放作为大多数桌面系统的默认视频交付方法。 基于Flash的HDS视频流仍可用作替代播放选项。
* 添加了对在Chrome浏览器下运行的鼠标和触摸输入的支持。 Microsoft Surface设备属于此类别，但是，Microsoft Edge无法处理带有5.5查看器的触控输入。

## 5.5.1的错误修复 {#section-c64001afa2aa460180141bc8dcd507ec}

* 在eCatalog查看器中支持从右到左方向。
* `tip=0,-1,0` 导致超出范围错误。
...
* `InitalFrame` 仅链接到多页面跨页中最左侧的页面。 在纵向模式下影响移动设备。
...
* 海报图像可能会遇到 `max size` 错误。 您可能需要提高图像提供发布的限制设置。
* 如果托管HTML页面是从外部服务器(而不是Scene7服务器)提供，则视频字幕需要设置公司规则集。 请联系技术支持以获取帮助。
* 由于缓冲，Analytics跟踪可能会报告不正确的播放百分比。
* 在iPad或Android设备上，可能会显示黑框而不是海报图像。
* 在iPad或Android设备上，在查看器加载期间，屏幕上可能会闪烁黑帧。
