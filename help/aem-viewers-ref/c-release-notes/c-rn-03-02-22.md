...
* 向音量滑块添加了ARIA支持。 <!--  (CQ-4324080) -->
* 添加了可视星号(*)符号 `To` 和 `From` “电子邮件”对话框中的必填电子邮件编辑字段。 <!-- (CQ-4290935) -->
* 使用向下箭头和表单模式快捷键修复了“电子邮件”对话框中的旁白标签信息 `F`. <!-- (CQ-4290934) -->
* 修复了以下项的错误建议： `To` 和 `From` 在“电子邮件”对话框中（位于上的“使用键盘”选项卡）。 <!-- (CQ-4290930) -->
* 修复了以下项的标签映射 **[!UICONTROL 嵌入大小]** 菜单按钮出现在 **[!UICONTROL 获取嵌入代码]** 表单模式快捷键中的对话框 `F`. <!-- (CQ-4290929) -->
* 中的空白页面标题 **[!UICONTROL 查看器预设]** 页面现已修复。 <!-- (CQ-4290936) -->
* 交互式视频编辑器中的单选按钮的对齐现已修复。 <!-- (CQ-4330159) -->
...
参见 [支持](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/intro/support.html#intro).
...
* 在表中嵌入查看器可能导致不正确调整查看器大小或将查看器置于非本机全屏模式。 Adobe建议您改用DIV。
* 代码中具有显式实例名称的参数要求URL中的实例名称也被覆盖(例如， `zoomView.iconfeffect=0`)。
* 图像服务命令 `crop` 不受支持。
* **[!UICONTROL 关闭]** 仅当在子窗口中打开查看器时，按钮才有效。
...
* 海报图像可能会遇到 `max size` 错误。 增加图像服务发布的限制设置。
* 如果从外部服务器(而不是Dynamic Media服务器)提供托管HTML页面，则视频字幕需要公司规则集。 联系人 [Adobe客户关怀](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/intro/support.html#intro) 以寻求帮助。
* Analytics跟踪可能会由于缓冲而报告不正确的播放百分比。
* 在iPad或Android™设备上，显示的是黑色框架，而不是海报图像。
* 在iPad或Android™设备上，加载查看器期间屏幕上会闪烁一个黑色框架。
* 在iPad设备上，当背景设置为白色/透明时，VideoPlayer组件的一侧会显示黑色边框。
* 在iPad上，使用iOS 7时，视频的最后一帧会被扭曲。
* 在Chrome、Firefox和Internet Explorer浏览器上，在HLS流模式下的视频搜寻期间会偶尔发生宏阻塞。
