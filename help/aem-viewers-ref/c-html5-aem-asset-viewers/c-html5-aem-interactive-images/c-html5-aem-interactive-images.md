---
title: 交互式图像
description: 交互式图像查看器是一种显示单个不可缩放图像的查看器，其中包含可单击的热点。 此查看器旨在实施“可购物横幅”体验。 也就是说，用户可以选择横幅图像上的热点，并重定向到您网站上的概览或产品详细信息页面。 它设计为可在台式机和移动设备上工作。
...
1. 创建和初始化查看器。
   ...
   以下示例用于创建查看器实例、将最低必要的配置选项传递给构造函数以及调用`init()`方法。 该示例假设`interactiveImage`是查看器实例；`s7viewer`是占位符`DIV`的名称；`http://aodmarketingna.assetsadobe.com/is/image`是图像服务URL，`/content/dam/mac/aodmarketingna/shoppable-banner/shoppable-banner.`是资源：
