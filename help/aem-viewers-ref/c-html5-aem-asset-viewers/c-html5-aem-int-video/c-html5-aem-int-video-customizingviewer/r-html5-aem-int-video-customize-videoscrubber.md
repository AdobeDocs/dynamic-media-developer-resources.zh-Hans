...
可以本地化视频洗刷工具提示。 有关详细信息，请参阅[用户界面元素的本地化](../../../c-html5-aem-asset-viewers/c-html5-aem-int-video/c-html5-aem-int-video-viewer-localization.md#concept-cbfc39344c494eb7b9f6a272cff0cc74)。
...
当使用`navigation`参数启用视频章节标记时，章节位置将作为标记显示在视频洗刷跟踪的顶部。
...
>[!NOTE]
>
>此按钮支持`state`属性选择器，您可以使用它来将不同的外观应用于不同的按钮状态。 特别是，`selected='default'`对应于默认的视频章节标记状态，当鼠标悬停在手势或触摸手势上激活视频章节标记时，将使用`selected='over'`。
