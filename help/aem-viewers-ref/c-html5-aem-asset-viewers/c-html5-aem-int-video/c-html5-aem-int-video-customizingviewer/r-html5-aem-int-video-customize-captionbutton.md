...
>[!NOTE]
>
>此按钮同时支持`state`和`selected`属性选择器，它们可用于将不同的外观应用于不同的按钮状态。 特别是，`selected='true'`对应于字幕可见时的状态，而隐藏字幕时使用`selected='false'`。
...
设置一个28 x 28像素的隐藏式字幕按钮。 按钮必须位于距离控制栏顶部四个像素和距离控制栏右边缘68个像素的位置。 此外，如果选择或未选择，它必须针对四种不同的按钮状态分别显示不同的图像。
