---
title: 全景查看器的JavaScript API引用
description: HTML5全景查看器的主类是PanoramicViewer。 它在s7viewers命名空间中声明。 此JavaScript API涵盖此特定类的构造函数、方法和回调。
solution: Experience Manager
feature: Dynamic Media Classic,Viewers,SDK/API,Panoramic
role: Developer,User
exl-id: null
source-git-commit: 8aebcacd5abdc23565aab1bc3506c36f055b6439
workflow-type: tm+mt
source-wordcount: '92'
ht-degree: 0%

---

# 全景查看器的JavaScript API引用{#javascript-api-reference-for-panoramic-viewer}

HTML5全景查看器的主类是PanoramicViewer。 它在s7viewers命名空间中声明。 此JavaScript API涵盖此特定类的构造函数、方法和回调。

在以下所有示例中， `<instance>` 表示从实例化的JavaScript查看器对象的实际名称 `s7viewers.PanoramicViewer` 类。
