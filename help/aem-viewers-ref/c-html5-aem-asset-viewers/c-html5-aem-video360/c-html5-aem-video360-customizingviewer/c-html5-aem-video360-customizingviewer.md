...
description: Video360查看器的所有可视化自定义和大多数行为自定义均通过创建自定义CSS来完成。
keywords: 响应式
...
Video360查看器的所有可视化自定义和大多数行为自定义均通过创建自定义CSS来完成。
...
创建自定义CSS时，请记住，查看器将`.s7video360viewer`类分配给其容器DOM元素。 如果您使用通过`style=`命令传递的外部CSS文件，请将`.s7video360viewer`类用作CSS规则的后代选择器中的父类。 如果您在网页上执行嵌入样式，则还可以使用容器DOM元素的ID限定此选择器，如下所示：
...
在应用于HTML5时，移动设备查看器使用四个CSS媒体查询（在CSS中定义），其顺序如下：
