...
範例 — 若要設定含邊框間距的對話方塊標頭，請以24 x 17畫素圖示和16點粗體標題顯示。 最後，還有28 x 28畫素的「關閉」按鈕，位於距離上方的兩個畫素，以及距離對話方塊容器右方的兩個畫素：
...
對話方塊頁尾包含「取消」和「傳送電子郵件」按鈕。 頁尾容器由下列CSS類別選擇器控制：
...
<table id="table_0AF7AAAB846A46D690896AFD68575669"> 
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p> 可用來從視覺上分隔頁尾與對話方塊其餘部分的邊框。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
範例 — 若要設定具有64 x 34 「取消」按鈕和82 x 34 「傳送電子郵件」按鈕的對話方塊頁尾。 最後，每個按鈕狀態的文字顏色和背景顏色會有所不同：
...
對話方塊標籤可以本地化。 另請參閱 [使用者介面元素的本地化](../../../c-html5-aem-asset-viewers/c-html5-aem-smartcropvideo/r-html5-aem-smartcropvideo-viewer-localization.md#concept-1d5ca2d8480f4064a51eddba13940aad) 以取得詳細資訊。
...
從標籤展開至對話方塊內文左邊緣到右邊緣的大部分輸入欄位（包括「寄件者」欄位和「訊息」欄位）都是透過下列方式控制：
...
「收件者」輸入欄位較窄，因為它會為右側的「移除電子郵件」按鈕分配空間。 它由下列CSS類別選取器控制：
...
範例 — 若要設定表單使其有一個畫素灰色邊框，且所有輸入欄位周圍有九個畫素的內距。 對於驗證失敗的欄位，要有相同的紅色邊框，要有250畫素寬的「至」輸入欄位，其餘輸入欄位有300畫素寬：
...
共用的內容會顯示在對話方塊內文底部，並包含縮圖、標題、來源URL和說明。 它會包裝在容器中，並受到下列控制：
...
當使用者輸入不正確的輸入資料且內嵌驗證失敗時，或者在提交表單時對話方塊必須呈現錯誤或確認訊息時，對話方塊內文的頂部會顯示訊息。 它由下列CSS類別選取器控制：
...
>[!NOTE]
>
>此訊息支援 `state` 屬性選擇器，其可能值如下： `verifyerror`， `senderror`、和 `sendsuccess`. 值 `verifyerror` 在因內嵌輸入驗證失敗而顯示訊息時設定。 值 `senderror` 在後端電子郵件服務回報錯誤時設定。 此 `sendsuccess` 值是在成功傳送電子郵件時設定。 如此一來，訊息的樣式可能會因對話方塊狀態而異。

錯誤訊息可以本地化。 另請參閱 [使用者介面元素的本地化](../../../c-html5-aem-asset-viewers/c-html5-aem-smartcropvideo/r-html5-aem-smartcropvideo-viewer-localization.md#concept-1d5ca2d8480f4064a51eddba13940aad) 以取得詳細資訊。

範例 — 若要設定訊息以使用10點粗體字型，請在左側使用25畫素行高和20畫素邊框間距。 此外，請使用驚歎號圖示，如果有錯誤則使用紅色文字，如果成功則使用無圖示和綠色文字：
