...
智能裁剪视频查看器表示一个主JavaScript文件和一组帮助程序文件 — 单个JavaScript包含此特定查看器使用的所有Viewer SDK组件、资源以及查看器在运行时下载的CSS。
...
建议您为弹出操作模式使用现成的HTML页面。 它名为[!DNL SmartCropVideoViewer.html]，位于标准IS-Viewers部署的[!DNL html5/]子文件夹下：
...
1. 正在将查看器JavaScript文件添加到您的网页。
   ...
   创建查看器需要您在HTML头中添加脚本标记。 在使用查看器API之前，请确保包括[!DNL SmartCropVideoViewer.js]。 [!DNL SmartCropVideoViewer.js]文件位于标准IS-Viewers部署的[!DNL html5/js/]子文件夹下：
