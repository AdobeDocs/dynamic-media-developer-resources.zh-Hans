...
查看器附带四个现成的CSS文件，用于数字和虚线设置指示符，每个文件采用“浅色”和“深色”颜色方案。 默认情况下使用“点光”版本，但使用不同的标准CSS并设置`SetIndicator.mode`参数可轻松切换到不同版本。 其他标准CSS位于以下位置：
...
创建自定义CSS时，请记住，查看器将`.s7carouselviewer`类分配给其容器 DOM元素。 如果您使用的是使用`style=`命令传递的外部CSS文件，请在CSS规则的子代选择器中使用`.s7carouselviewer`类作为父类。 如果要在网页上添加嵌入样式，则还可以通过以下容器DOM元素的ID来限定此选择器：
