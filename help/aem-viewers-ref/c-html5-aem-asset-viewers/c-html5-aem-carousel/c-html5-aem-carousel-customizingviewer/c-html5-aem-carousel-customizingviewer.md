---
description: 旋转式查看器的所有可视自定义和大多数行为自定义都是通过创建自定义CSS完成的。
keywords: responsive
seo-description: 旋转式查看器的所有可视自定义和大多数行为自定义都是通过创建自定义CSS完成的。
seo-title: 自定义传送查看器
...
旋转式查看器的所有可视自定义和大多数行为自定义都是通过创建自定义CSS完成的。
...
查看器提供四个现成的CSS文件，分别用于数字和点线设置指示符，每个都采用“浅色”和“深色”颜色方案。 默认情况下，使用“点光”版本，但使用不同的标准CSS并设置`SetIndicator.mode`参数可轻松切换到不同版本。 其他标准CSS位于以下位置：
...
创建自定义CSS时，请牢记查看器将`.s7carouselviewer`类分配给其容器DOM元素。 如果使用使用通过`style=`命令传递的外部CSS文件，请在CSS规则的子代选择器中使用`.s7carouselviewer`类作为父类。 如果要在网页上添加嵌入样式，则还可以通过容器DOM元素的ID来限定此选择器，如下所示：
...
以下范例CSS将鼠标输入系统上的按钮大小设置为28 x 28像素，将触屏输入设备上的按钮大小设置为56 x 56像素。 如果查看器的大小更小，则设置为20 x 20像素。
...
下面是一个基于“sprite”的示例，用于设置同一热点图标的样式：
