...
按钮仅在 `CarouselViewer.autoplay` 参数设置为 `1`；否则，它会隐藏。 使用CSS，您可以相对于包含该按钮的控制栏调整其大小、外观和位置。
...
例如，假设您要设置一个28 x 28像素的播放暂停按钮。 您想要将按钮定位在距离查看器底部17像素和距离其左边缘12像素的位置。 此外，您希望在选中或未选中时，为四种不同的按钮状态分别显示不同的图像。
