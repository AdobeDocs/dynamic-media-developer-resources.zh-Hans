---
title: Scene7发布系统6.9发行说明（2014年10月）
description: “最新发行说明介绍了Adobe Scene7 Publishing System 6.9(Adobe Experience Cloud中Adobe Experience Manager解决方案的一部分)的最新功能、增强功能和修复。”
...
# Scene7发布系统6.9发行说明（2014年10月）{#scene-publishing-system-release-notes-october}

最新发行说明介绍了Adobe Experience Cloud中Adobe Scene7解决方案的Adobe Experience Manager Publishing System 6.9部分的最新功能、增强和修复。

## Scene7发布系统6.9发行说明（2014年10月） {#topic-1d3011ecca71488abe0d8156a9fc1b97}

最新的发行说明描述了的最新功能、增强和修复。 [!DNL Adobe Scene7 Publishing System] 中的Adobe Experience Manager解决方案的6.9部分 [!DNL Adobe Experience Cloud].

* [Scene7 Publishing System 6.9发行亮点](s7rn69.md#section-2ed72b7902be4509876d81f25b5632ce)
...
## Scene7 Publishing System 6.9发行亮点 {#section-2ed72b7902be4509876d81f25b5632ce}
...
* **Visual Configurator的安全预览**  — 在将个性化体验推送到生产环境之前，安全地预览和测试这些体验的可视化渲染。
* **SSO（单点登录）功能**  — 现在，您可以从Adobe Experience Cloud无缝启动Scene7，并使用Adobe ID登录到Scene7。
...
* 一款新的开箱即用的“自适应视频”编码预设，它现在可与任何纵横比配合使用，以创建用于交付到移动设备、平板电脑和台式机的视频。 使用此新预设编码的上传源视频将设置为固定高度，而宽度会自动缩放以保留视频的长宽比。 请参阅[用于编码视频文件的视频预设](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/setup/application-setup.html?lang=en#setup)。
* 提高了弹性可扩展性和编码性能。
* 视频字幕现已开箱即用 `Universal_HTML5_MixedMedia_dark` 和 `Universal_HTML5_MixedMedia_light` 查看器。 请参阅[在视频中添加题注](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/video/adding-captions-video.html#video)。
...
* 渲染服务器请求的安全测试 — 核心图像服务资源和视频资产的安全测试现在已扩展到也包括晕影。 请参阅 [在公开资产之前测试资产](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public.html#upload-publish).
* 监控失效CDN缓存功能。 选择CDN失效模板后(请参阅 [常规设置](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/setup/application-setup.html#general-settings))，以及 **[!UICONTROL 使CDN失效]** 发出请求后，现在会弹出一个指示器，估计清除缓存所需的时间。
...
* 固定 — 如果图像已移动，则将其右移 `DIV` 具有 `text-align:center` 样式。
...
**Dynamic Media Image Serving Viewer 5.1.1的新增功能、增强功能和错误修复**
...
将计算机设置为访问暂存服务器后，您可以检查网站以测试升级。 对于使用现成HTML5查看器的客户，最佳做法是针对Adobe的标准暂存服务器和s7is1-preview-staging.scene7.com进行测试。
...
* HTML5 MixedMedia查看器：交换资源类型时，缩放按钮状态未重置。
...
* 无法预览现成的HTML5视频查看器预设。
* 即使重写了规则，Scene7 Publishing System仍会记住该组。
* Scene7 Publishing System没有正确显示某些重复正则表达式代码。
* 如果未选择任何预设，则在上传时应用所有批次集预设。

## 不再支持的功能 {#section-b061ad3d170a45ee8e694f997fd5d8ef}

* 由于Scene7应用程序不支持Creative Suite面板，因此此版本中已弃用Creative CloudFlash扩展。 内容摄取工作流的进一步开发重点在于Adobe Experience Manager Dynamic Media和Creative Cloud。 对于Scene7，Scene7 Publishing System中的上传和导出功能仍可保留并改用。
...
* 提醒：自2014年1月31日起，Scene7正式停止支持DHTML查看器平台。
