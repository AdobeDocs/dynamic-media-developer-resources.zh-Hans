...
description: “ 2016年秋季版Adobe Scene7的最新发行说明，是Adobe Experience Cloud中Adobe Experience Manager解决方案的一部分。”
...
Adobe Scene7 2016年秋季版本部分的最新发行说明，内容涉及Adobe Experience Cloud中的Adobe Experience Manager解决方案。

## Scene7 2016年秋季版本 {#topic-791cdf80f91e457fbb63bfedf79f5a94}

的最新发行说明 [!DNL Adobe Scene7] 2016年秋季版本的一部分 [!DNL Adobe Experience Manager] 解决方案 [!DNL Adobe Experience Cloud].
...
* [Dynamic Media Classic图像提供6.3.2和图像呈现6.3.2](s7rnfall2016.md#section-19a3e96f52c74757bcdea0f8a11001f2)
...
* 删除了 [!DNL Adobe Scene7 Publishing System] 用户界面。
* 在必要和可能的情况下，为所有Scene7 Servlet添加了身份验证
* 修复了与垃圾桶中的列表视图有关的错误。
* 已删除 **创建Dynamic Media Classic(Scene7)管理员** 用户管理中的用户功能。
...
* 添加了对查看器分析的Experience Cloud组织ID的支持。
* 已实施Scene7 SAML使用者。
...
* 修复了 `tip=0,-1,0` 导致超出范围错误。

**兼容性说明**

* BlackBerry®

   * 与旧版AVS集不兼容。 客户端必须重新上传AVS集才能允许播放。

* 常规

   * 当用户放大到页面时，浏览器端缩放可能会导致用户界面和图像变得模糊。 UI格式设置可能也会因缩放而显示不正确。 这种效果全屏传播。
   ...
   * “关闭”按钮不适用于iOS 8.0和8.1，但不再出现在iOS 8.2中

* 银河二世

   * 在缩放和eCatalogHTML5查看器中出现内存泄漏。 在框架中重复导航可能会导致浏览器崩溃。
...
* HTC移动设备
   ...
   * HTC移动设备Adobe的发现显示，无法禁用本机捏合缩放是HTC UI包装器(HTC Sense)的“功能”。 当在查看器上使用“捏合缩放”手势时，此问题可能会导致整个页面缩放。 建议改用双击。
...
* 图像服务修饰符 `iscommands` 未添加到 `req=set` 按设计请求。 仅影响图像显示的修饰符可以正常工作。 影响大小的修改量必须用于复杂资产。 例如，

   `https://s7d9.scene7.com/s7viewers/html5/BasicZoomViewer.html?asset= {Scene7SharedAssets/Backpack_B?extendn=0.5%252C0.5%252C0.5%252C0.5}`

* [弹出] 鼠标关闭后，IE9有时会停留在屏幕上。
* 浏览器缩放会导致调整大小错误。
* iPad 2:大型eCatalog资产使iOS上的Safari崩溃。
* 所有查看器

   * 不支持水印、模糊处理和锁定。
   * 不支持图像预设。
   * 使用 `display:none` 当前不支持CSS或通过从父节点动态分离CSS。
...
* HTML5所有查看器
   ...
   * 的 `iscommands` 修饰符不支持影响图像大小的图像服务修饰符。

* HTML5 eCatalog

   * 导航到其他HTML页面并返回有时会导致查看器重置回第一页。
...
* HTML5视频
   ...
   * 海报图像可能会遇到“最大大小”错误。 公司必须提高图像服务发布的限制设置。
   * 如果托管HTML页面是从外部服务器(而不是Scene7服务器)提供的，则视频字幕需要公司规则集。 请联系Adobe支持以获取帮助。
...
* 目录 —   `initialframe` 对HTML5 eCatalog查看器的移动设备上的纵向模式没有影响。
...
* FlyoutzoomView
   ...
   * 修复了以下情况 `tip=0,-1,0` 导致超出范围错误。

**兼容性说明**

* Android™ 4.x

   * 要禁用默认设置，必须为组件添加以下CSS规则，请用蓝色突出显示：
...
* iPad 3
   ...
   * iPad 3的本机分辨率为2048x1536。 如果公司的IS发布设置为小于图像大小限制，则此解决方案可能会导致显示问题。
...
* 触控设备
   ...
   * 自定义游标在触屏设备上不起作用。
   * 当前不支持在触控设备上缩放页面。 嵌入HTML5查看器需要具有相应设置的视区元标记。
...
* 所有组件
   ...
   * 在版本2.7.2及更早版本中，使用 `insertBefore()` API。 因此，无论何时创建组件实例相对于其他组件，这些组件都会置于堆叠顺序的底部。 在2.8.1版本中，所有组件都使用 `appendChild()` API现在，这意味着组件堆叠顺序与实例创建顺序匹配。
...
* PageScrubber组件
   ...
   * 在iPhone 5中，当PageScrubber气泡设置为文本时，在沿着轨道滑动按钮时，会显示伪像。 使用 `-webkit-background-clip: content;` 在样式中可以解决问题。
...
* IC实用程序 —  `downsample2x2` 标记不再受支持。 此标记质量较差，IPS不再使用2x2下采样器。
* CORS标头 — 当前，为 `/is/content/` 请求。
