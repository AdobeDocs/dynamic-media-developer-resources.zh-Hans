...
description: 最新发行说明介绍了Adobe Scene7 Publishing System 6.8（Adobe Experience Cloud中Adobe Experience Manager解决方案的一部分）的最新功能、增强和修复。
...
最新发行说明介绍了Adobe Experience Cloud中Adobe Experience Manager解决方案的Adobe Scene7 Publishing System 6.8部分的最新功能、增强和修复。

## Scene7 Publishing System 6.8发行说明（2014年5月） {#topic-4517aab837ea4d6d8204425958860e5a}

最新发行说明介绍了[!DNL Adobe Scene7 Publishing System]中Adobe Experience Manager解决方案的[!DNL Adobe Experience Cloud] 6.8部分的最新功能、增强和修复。
...
* 为响应式Web设计添加了支持 — 基于断点的响应式图像服务和响应式HTML5查看器（图像集、视频、混合媒体、eCatalog、旋转集、样本集、缩放），它们可根据页面布局动态调整大小。
...
* 向HTML5缩放查看器和HTML5混合媒体查看器添加了`SetIndicator`，用于以可视方式显示移动设备上的色板。
...
* 用于HTML5查看器的`initComplete`处理程序，用于在查看器后触发函数。
* HTML5查看器的Analytics处理程序，用于支持第三方跟踪。
...
* 为了防止列入阻止列表，请将忘记密码电子邮件从HTML转换为纯文本。
* 删除了默认角色。
* 除了能够通过服务器目录上传(ftp)和上传URL将内容上传到Scene7 Publishing System之外，您现在还可以使用HTTP POST上传内容。 有关使用HTTP POST的更多信息，请参阅[Image Production System API](/help/aem-ips-api/c-overview.md)。
...
* HTML5 eCatalog Viewer鼠标拖放功能在包含图像映射的页面的Internet Explorer中不起作用。
* HTML5 Video Viewer阻止访问解锁Flash插件。
* HTML5查看器阻止在Firefox浏览器上打印。
* 启动或重置缩放时，HTML5缩放查看器显示轻微抖动。
* HTML5和AS3查看器支持HTTPS。
* autoResize不应位于HTML弹出查看器复制URL/嵌入代码中。
* HTML5 Video Viewer Copy URL中没有videoserverurl修饰符。
* 参数`contenturl`来自发布服务器HTML5查看器预览URL。

**映像**

* 无法在图像发布设置中将已调整视图添加为默认图像。
...
* 初始化公司设置时，Scene7 Publishing System在API `<defaultValue>`中发送重复选项`<createMetadataField>`。

* Flash插件和Scene7 Publishing System由于许多文件夹的Flash内存不足而崩溃。
* 删除用户定义的日期/时间字段的值时，Scene7 Publishing System不会向IPS（图像发布系统）发送请求。
* 如果启用了Evideo编码选项，则重新处理PDF或PSD文件时遇到自适应视频编码警告消息。
* 启用SSL后无法导出资源。
* 忘记密码电子邮件中显示的密码不正确。
* 任何Scene7 Publishing System Web管理员请求都会被重定向到HTTP。
* 如果未选择预设，则在上载期间应用所有批次集预设。
* 上传s7info内容时，图像生成系统通过HTTP请求s7info。
* 忘记密码电子邮件中的URL是HTTP，即使它来自SSL Scene7 Publishing System也是如此。
* 在HTML格式的“忘记密码”电子邮件中，对特殊字符进行了编码，导致密码显示不正确。
...
* Adobe Illustrator和Adobe InDesign2014年5月更新预计会导致与创建Scene7 FXG的插件不兼容。 这些不兼容性会影响Scene7 Web到打印解决方案的用户。 如果您使用Scene7 Web-to-Print，请勿将Illustrator和InDesign升级到此Creative Cloud更新。 请继续使用2014年5月之前的版本。 Adobe Inc预计在2014年秋季将Scene7 FXG创建与较新版本的Illustrator和InDesign恢复兼容性。
...
* Scene7 6.9秋季版本中不支持文本断字。 断字词典定义如何拆分和断字字以支持最佳文本流。
...
* Scene7 Creative Suite Extensions (Flash Panels)已于2014年底被弃用。 内容摄取工作流的进一步开发重点在于Adobe Experience Manager Dynamic Media和Creative Cloud。 对于Scene7，可以保留Scene7发布系统中的上传和导出功能并改用。
...
* 已修复：当嵌入到`selectSwatch`函数中时，`initViewer()`方法未触发。
