...
最新的发行说明描述了的最新功能、增强和修复。 [!DNL Adobe Scene7 Publishing System] 中的Adobe Experience Manager解决方案的6.8部分 [!DNL Adobe Experience Cloud].
...
* 已添加 `SetIndicator` HTML5缩放查看器和HTML5混合媒体查看器，以可视化方式表示移动设备上的色板。
* 已添加 `getComponent` 此API允许外部访问HTML5查看器组件。
...
* 删除了默认角色。
* 除了能够通过服务器目录上传(ftp)和上传URL将内容上传到Scene7 Publishing System之外，您现在还可以使用HTTPPOST上传内容。 请参阅 [图像生产系统API](/help/aem-ips-api/c-overview.md) 有关使用HTTPPOST的更多信息。
...
* 已修复： `selectSwatch` 方法嵌入到以下位置时未触发 `initViewer()` 函数。

**社交共享**

* 添加了对承载修饰符的支持，以控制共享按钮的布局位置。
* 添加了用于控制在与社交功能交互时暂停/播放的事件。
* 已修复：社交共享图标不会因 `controlbar` 在“缩放”上。
...
* 添加了对的支持 `initialBitRate` 修饰符，用于选择在桌面上初始播放时使用的比特率。
