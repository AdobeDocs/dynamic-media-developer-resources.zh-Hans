...
文档/文字样式用于所有输入和输出消息，这些消息的类型基于XML架构定义语言([https://www.w3.org/TR/xmlschema-0/](https://www.w3.org/TR/xmlschema-0/))并在WSDL文件中指定。 所有类型都需要使用WSDL文件中指定的目标命名空间值来限定名称。
...
<table id="table_B295FEB0EA584C2BA4122FC084AEF18D"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> <p> <span class="codeph">区域设置</span> </p> </td> 
...
`authHeader`元素始终在命名空间`http://www.scene7.com/IpsApi/xsd`中定义，无论API版本如何。
...
此样式仍支持向后兼容，但已弃用，推荐使用`authHeader`元素。
